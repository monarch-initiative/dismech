...
4. **Filter to sole-assignee issues.** Drop any issue whose `assignees` array has more than one entry. The user explicitly wants solo work — an issue assigned to both `alice` and `bob` is a team task, even if the current user is `alice`. Do this filter in jq, not by hand:
   ...
   ```
5. **Filter out issues with an existing PR.** For each remaining issue number `N`, check both directions:
   ```bash
   # Forward: PRs whose body references this issue with a linking keyword
   gh pr list --state all --search "closes #${N} OR fixes #${N} OR resolves #${N}" \
     --json number,state,title

   # Backward: PRs GitHub has auto-linked from the issue's development sidebar
   gh issue view "${N}" --json closedByPullRequestsReferences \
     -q '.closedByPullRequestsReferences | length'
   ```
   Skip the issue if either query returns a non-empty result. A merged PR usually means the issue should already be closed, but stale links happen — be conservative. If `closedByPullRequestsReferences` is rejected as an unknown field by the local `gh` version, fall back to the search query alone and report the gh version downgrade in your final message.
...
8. **Dispatch /curate in parallel.** In a **single message**, launch N Agent tool calls — one per disease — using `subagent_type: claude` so each agent has full tool access. Each agent's prompt MUST:
   ...
   Dispatching all N in one message is the whole point — sequential dispatch defeats the skill. See `superpowers:dispatching-parallel-agents` if you need a refresher on parallel sub-agent semantics.
...
| PR linking check | `gh pr list --state all --search "closes #<N>" --json number,state` |
| Dispatch | Single message with N `Agent` calls, `subagent_type: claude` |
...
- **Skipping the PR-existence check.** If a contributor opened a PR three days ago and it is still under review, re-running `/curate` will overwrite their work and waste the reviewer's time. Both the forward (`gh pr list --search`) and backward (`closedByPullRequestsReferences`) checks are mandatory.
