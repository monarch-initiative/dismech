...
## Scope and cell of origin

Before selecting a cancer entry boundary or marking its origin, read
[scope and cell of origin](references/scope-and-origin.md). It contains the
ratified granularity ladder, rules for promoting biomarker strata, somatic
versus exposure initiation, and the derived cell-of-origin checks.
Do not create stage-specific entries or add a standalone cell-of-origin slot.

Use `medical-action` for clinical actions and agents, `pathograph` for
atomic causal chains, and `create-module` for hallmark conformance.
