...
def create_item(
    project_number: int, owner: str, item: CheckboxItem, dry_run: bool
) -> bool:
...
def sync_project(
    filepath: Path, project_number: int, owner: str, dry_run: bool = False
):
    ...
    status_field = next(
        (f for f in fields.get("fields", []) if f["name"] == "Status"), None
    )
    ...
    for item in md_items:
        if item.name in gh_items:
            ...
            if (
                ...
                and status_field
                and target_status in status_options
            ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Sync markdown project to GitHub Project"
    )
