...
### Step 3b: GeneReviews baseline

For a Mendelian entry, read [the baseline workflow](references/genereviews-baseline.md)
and run `just check-genereviews <file>`. Compare the chapter's clinical
features with the entry and source frequency and safety claims separately.
StatPearls is context, not a mandatory baseline.

### Step 4: Enhance YAML with evidence

Use `pathograph` for mechanism chains, targets, scale, and environmental links;
`medical-action` for treatments and other clinical actions; `model-curation`
for model records; and `create-module` for conformance. Read
[clinical sections](references/clinical-sections.md) when adding inheritance,
subtypes, laboratory intervals, prevalence, or gene case fractions.
Use `dismech-references` and `dismech-terms` for the evidence and binding
procedures; their supporting references own detailed validation guidance.
