...
def render_alternatives(slug, entry):
    ...
    count = len(solutions)
    title = "top five" if count == 5 else f"all {count}"
    lines = [
        f"# {slug}: {title} distinct solutions",
        "",
        ...
        if count == 5
        else "All consistent solutions are shown.",
        "",
    ...
    lines += [
        ...
        f"The posterior denominator includes all **{baseline['distinct_solutions']}** satisfiable assignments"
        + (", not just these five." if count == 5 else "."),
        "",
        ...
        "",
        "| Hypothesis | Mapping | Input prior P(true) | Marginal posterior P(true) | "
        + " | ".join(f"S{i + 1}" for i in range(count))
        + " |",
        "|---|---|---:|---:|" + "---|" * count,
    ]
