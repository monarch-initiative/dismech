...
`.claude/skills/typesafe-ai/SKILL.md` when working on AI-powered features in
...
For Codex and any other agent: `cache/dataset_accessions.json` is retired and
deleted. Never read, recreate, refresh, or restore it. If an old PR carries it,
keep its deletion when resolving conflicts; do not merge its contents. Dataset
...
that file instead. The shared blob was a single sorted JSON file that every
