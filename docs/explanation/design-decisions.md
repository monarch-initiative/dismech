...
- **Model below target** — upward extrapolation. The model cannot observe the outcome it
  is cited for. This is the reviewable state, and `check_upward_extrapolating_links_are_caveated`
...
| Spatial, temporal and comparator context on `ExperimentalReadout` | **Open; no schema change proposed yet.** A readout records *what* was measured and *which way it went*, but not where in the tissue, at what temporal resolution, with what instrument, or against which comparator arm — an exhaustive search of the schema finds no slot on this path, and the nearest analogues (`spatial_extent`, `platform`) belong to descriptors and datasets respectively. This bites hardest for instrumented organoid and organ-chip models, where an *internal* spatial comparator (organoid interior vs. edge) is the methodological point and currently survives only as `interpretation` prose. Weigh against two facts: `assays` is already defined and OBI-bound yet populated on 0 of 289 readouts (OBI has no adapter in `conf/oak_config.yaml`, so it cannot be validated), and 230 `modeled_mechanisms` links carry no `relationship` at all — the expressive slots that exist are under-filled. The narrowest tractable piece is naming the comparator, since `ModelReadoutDirectionEnum` is *defined* relative to "the model's control or comparator arm" that nothing records. | [Spatially resolved in vitro models](../reports/spatially-resolved-in-vitro-models-2026-09-02.md) |
