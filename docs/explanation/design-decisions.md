...
| Investigation-readout phenotype backfill (`reports_on`) | New lean `PhenotypeReadout` slot added (§10): investigation-result phenotypes (abnormal ERG/EEG, `Elevated circulating … concentration`) can now attach to the mechanism they measure via a dashed observational readout edge instead of floating as orphan nodes or being mis-wired as causal `downstream` edges. `Bardet-Biedl_Syndrome` (Abnormal electroretinogram → Photoreceptor outer-segment transport defect) is the worked exemplar. **Open:** backfill the remaining ~200 `Elevated/Decreased circulating … concentration` and other test-result phenotypes, and audit existing `downstream` edges that target a test-result phenotype for the same causal→readout correction. No automated finder yet (candidate: an HP-ancestry check against the abnormal-lab/investigation subtrees). | KB migration follow-up |
