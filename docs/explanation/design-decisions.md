...
| SEPIO evidence representation in the core schema | **Experimental pilot, not a decision.** The experiment-grounded-evidence row above records "SEPIO reserved for the export layer", by analogy with the export-layer-only BioLink rule (§5). This pilot deliberately probes that boundary: it adds SEPIO's `EvidenceLine` / `DataItem` / `Document` **into `dismech.yaml` itself**, additively, next to the untouched native `EvidenceItem`. The case for doing so rather than exporting is that two of the three things SEPIO buys us — grouping several passages into one argument, and typing the source document — are *inputs* a curator has to author, not derivations a converter could compute from native evidence; an export-only mapping cannot invent information the source model never held. The third (strength separate from direction) is likewise a curator judgement. **Not settled**: whether these earn their keep against the added curation burden, and whether a lossy export-layer mapping would have been enough. Scoped to `Pathophysiology` and `Phenotype`, with one fixture under `kb/experimental/`. Revisit before any of it reaches `kb/disorders/`. | [#7439](https://github.com/monarch-initiative/dismech/issues/7439) · [sepio-evidence-pilot](sepio-evidence-pilot.md) |
