...
| `Experiment` class shape / cloud-lab execution | Design proposal, **not yet a schema change.** `Experiment` is one flat class reached from `Discussion.proposed_experiments`, and the fill rates across 2,378 experiments show it does not fit what curators write: `experiment_type` 15.5%, `model_systems` 5.7%, `assays` 0.5%, against `description` at 99.7% — and of 368 `experiment_type` values there are 330 distinct strings and exactly one ontology CURIE, curators hand-rolling the taxonomy the schema declined to give them. (The documented "prefer OBI terms" instruction is currently unfollowable: OBI is absent from `conf/oak_config.yaml` and has no `cache/enums/` membership cache, the same gap already recorded for `assays:`.) Proposal: an abstract base carrying the type-agnostic slots plus `is_a` subclasses per **design family** (perturbation screen / human observational / interventional trial / in-silico), whose discriminating slots are near-disjoint; an `executability` ladder (`CONCEPTUAL` / `SPECIFIED` / `PARAMETERIZED` / `EXECUTABLE`, where all 2,378 are today `CONCEPTUAL`); and a typed `protocol_binding` handle to an external LabOP/Autoprotocol/protocols.io document. **Key boundary:** execution detail (reagents, plate maps, volumes) is protocol engineering, not disease knowledge, and does not belong in `kb/` — it would duplicate across every entry that screens a library. What dismech owns is the pre-registered decision rule bound to a named causal edge, so hardening `decision_criterion` (free text, 30.5% filled) is worth more than any reagent slot — an asymmetry against the already-hardened `would_support` (#9224) that is backwards. **Fixes a live bug independently:** `Experiment.model_systems` has range `ExperimentalModel` (non-animal NAMs only), so animal, human-cohort and in-silico proposals all leak into `experimental_model_type: OTHER` — 38 of 135 — which `ExperimentalModel`'s own comment forbids; #8199 closed this for realized models but never for proposed ones. **Must be co-designed with the Experiment-grounded evidence row above:** those are the same object prospective vs retrospective (`experiment.design` is this row's design-family enum; `experiment.readout` is `Experiment.readouts`), and designing them separately would yield two vocabularies for one concept. | [The Experiment Model](experiment-model.md) · [`projects/AUTONOMOUS_LABS.md`](../../projects/AUTONOMOUS_LABS.md) |
