...
| Deprecated `prevalence.percentage` cleanup | `percentage` superseded by structured prevalence slots (§8) and deprecated. Of the 199 unit-ambiguous bare-number records, `scripts/resolve_bare_prevalence.py` auto-resolved **91** under the percent reading (low-value rare-disease prevalences, evidence-corroborated, additive); **108 remain for manual review** in `research/prevalence_bare_number_report.md`, split into: `MISPLACED_STAT` (35 — records that are not population prevalence at all but survival rates, staging fractions, complication rates, or fraction-of-category, wrongly placed in the prevalence slot — these need relocating, not just unit-fixing), `CONFIRMED_HIGH` (51 — high-percent values mixing legitimate common-disease prevalences with residual fraction-of-category leaks), `DISAGREE` (10 — percent reading conflicts with the evidence rate; some are genuine proportion/per-100k encodings e.g. Hirschsprung, Cockayne, Nephronophthisis), and `NO_EVIDENCE` (12). Plus ~8 free-prose head-counts. Field removal is deferred until the backlog clears. | migration follow-up |
