...
| Hypothesis-based phenotype algorithms | **Proposal drafted, not enacted.** `definition_type: PHENOTYPE_ALGORITHM` today implicitly assumes established/validated grounding. Proposal adds an orthogonal `derivation_basis` (`ESTABLISHED_CRITERIA` / `MECHANISTIC_HYPOTHESIS` / `MODEL_SYSTEM_EXTRAPOLATION`), reuses `hypothesis_group_id` to wire a `Definition` to the `mechanistic_hypotheses[]` entry it operationalizes, and adds `validation_status` (`PROPOSED` / `UNVALIDATED` / `VALIDATED_AGAINST_GOLD_STANDARD`) — so a mechanism-predicated EHR case-finding query (e.g. fever-triggered arrhythmia surfacing latent CACNA1C carriers) is not conflated with a consensus/OHDSI-validated phenotype. Includes a proposed foreign-key test and renderer badge. See [hypothesis-based-phenotype-algorithms.md](../hypothesis-based-phenotype-algorithms.md). Awaiting `@cmungall` sign-off before touching the schema. | [#6245](https://github.com/monarch-initiative/dismech/issues/6245) |
