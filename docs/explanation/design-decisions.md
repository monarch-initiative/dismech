...
| Deprecated `prevalence.percentage` cleanup | `percentage` superseded by structured prevalence slots (§8) and deprecated. Of the 199 unit-ambiguous bare-number records, **147 are resolved**: `scripts/resolve_bare_prevalence.py` auto-converted **91** under the percent reading (low-value rare-disease prevalences, evidence-corroborated, additive); **9** of the 10 `DISAGREE` records were hand-fixed (7 extractor noise where percent was right; Cockayne `4e-06` and Hirschsprung were genuine proportion/per-10k-birth encodings corrected from their evidence); and **47** of the 51 `CONFIRMED_HIGH` (high-percent) records were converted after per-record review — legitimate population/cohort prevalences whose `population` field qualifies them (incl. conditional ones like cholesteatoma within otitis-media patients, PTSD in conflict zones). **52 remain for manual review** in `research/prevalence_bare_number_report.md`: `MISPLACED_STAT` (35) plus 4 fraction-of-category records the keyword filter missed (a disease that is a *subtype* of its stated population — Mantle-Cell Lymphoma "3-10% of lymphomas", germ-cell tumour "of ovarian cancers", Posterior MI "of ACS", Campylobacteriosis "of cases") — all of these are not population prevalence and need relocating, not unit-fixing; `NO_EVIDENCE` (12); and `DISAGREE` (1 — Nephronophthisis `0.1-1.0`, neither a clean percent nor proportion, no corroborating evidence). Plus ~8 free-prose head-counts. Field removal is deferred until the backlog clears. | migration follow-up |
