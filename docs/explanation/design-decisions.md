...
| Deprecated `prevalence.percentage` cleanup | `percentage` superseded by structured prevalence slots (§8) and deprecated. Of the 199 unit-ambiguous bare-number records, **100 are resolved**: `scripts/resolve_bare_prevalence.py` auto-converted **91** under the percent reading (low-value rare-disease prevalences, evidence-corroborated, additive), and **9** of the 10 `DISAGREE` records were hand-fixed (7 were extractor noise where percent was right; Cockayne `4e-06` and Hirschsprung were genuine proportion/per-10k-birth encodings corrected from their evidence). **99 remain for manual review** in `research/prevalence_bare_number_report.md`: `MISPLACED_STAT` (35 — not population prevalence at all but survival rates, staging fractions, complication rates, or fraction-of-category wrongly placed in the prevalence slot — these need relocating, not unit-fixing), `CONFIRMED_HIGH` (51 — high-percent values mixing legitimate common-disease prevalences with residual fraction-of-category leaks), `NO_EVIDENCE` (12), and `DISAGREE` (1 — Nephronophthisis `0.1-1.0`, which is neither a clean percent nor proportion and has no corroborating evidence rate). Plus ~8 free-prose head-counts. Field removal is deferred until the backlog clears. | migration follow-up |
