

# Slot: disease_b 


_Second disease in a comorbidity pair_





URI: [dismech:slot/disease_b](https://w3id.org/monarch-initiative/dismech/slot/disease_b)
Alias: disease_b

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComorbidityAssociation](../classes/ComorbidityAssociation.md) | An association between two conditions, including directionality, evidence, an... |  no  |






## Properties

* Range: [ConditionDescriptor](../classes/ConditionDescriptor.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:disease_b |
| native | dismech:disease_b |




## LinkML Source

<details>
```yaml
name: disease_b
description: Second disease in a comorbidity pair
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: disease_b
domain_of:
- ComorbidityAssociation
range: ConditionDescriptor
inlined: true

```
</details>