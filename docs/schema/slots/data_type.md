

# Slot: data_type 


_The type of omics or other data in the dataset_





URI: [dismech:slot/data_type](https://w3id.org/monarch-initiative/dismech/slot/data_type)
Alias: data_type

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Dataset](../classes/Dataset.md) | A reference to a publicly available omics or phenotype dataset |  no  |






## Properties

* Range: [DatasetTypeEnum](../enums/DatasetTypeEnum.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:data_type |
| native | dismech:data_type |




## LinkML Source

<details>
```yaml
name: data_type
description: The type of omics or other data in the dataset
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: data_type
domain_of:
- Dataset
range: DatasetTypeEnum

```
</details>