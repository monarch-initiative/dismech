

# Slot: variables 


_Variables/outputs of a computational model with ontology mappings_





URI: [dismech:slot/variables](https://w3id.org/monarch-initiative/dismech/slot/variables)
Alias: variables

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComputationalModel](../classes/ComputationalModel.md) | A computational or in-silico model relevant to understanding disease mechanis... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [ModelVariable](../classes/ModelVariable.md) |
| Domain Of | [ComputationalModel](../classes/ComputationalModel.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
| Multivalued | Yes |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:variables |
| native | dismech:variables |




## LinkML Source

<details>
```yaml
name: variables
description: Variables/outputs of a computational model with ontology mappings
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: variables
domain_of:
- ComputationalModel
range: ModelVariable
multivalued: true
inlined: true
inlined_as_list: true

```
</details>