

# Slot: go_terms 


_GO term enrichment results_





URI: [dismech:slot/go_terms](https://w3id.org/monarch-initiative/dismech/slot/go_terms)
Alias: go_terms

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [GOEnrichment](../classes/GOEnrichment.md) | GO enrichment results for an association signal |  no  |






## Properties

* Range: [GOEnrichmentTerm](../classes/GOEnrichmentTerm.md)

* Multivalued: True




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:go_terms |
| native | dismech:go_terms |




## LinkML Source

<details>
```yaml
name: go_terms
description: GO term enrichment results
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: go_terms
domain_of:
- GOEnrichment
range: GOEnrichmentTerm
multivalued: true
inlined: true
inlined_as_list: true

```
</details>