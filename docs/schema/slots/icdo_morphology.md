

# Slot: icdo_morphology 


_ICD-O morphology classification (for neoplastic diseases)_





URI: [dismech:slot/icdo_morphology](https://w3id.org/monarch-initiative/dismech/slot/icdo_morphology)
Alias: icdo_morphology

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [DiseaseClassifications](../classes/DiseaseClassifications.md) | Container for all classification assignments for a disease |  no  |






## Properties

* Range: [ICDOMorphologyAssignment](../classes/ICDOMorphologyAssignment.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:icdo_morphology |
| native | dismech:icdo_morphology |




## LinkML Source

<details>
```yaml
name: icdo_morphology
description: ICD-O morphology classification (for neoplastic diseases)
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: icdo_morphology
domain_of:
- DiseaseClassifications
range: ICDOMorphologyAssignment
inlined: true

```
</details>