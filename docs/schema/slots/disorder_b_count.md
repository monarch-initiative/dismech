

# Slot: disorder_b_count 


_Number of records/patients carrying disorder B in the source dataset_





URI: [dismech:slot/disorder_b_count](https://w3id.org/monarch-initiative/dismech/slot/disorder_b_count)
Alias: disorder_b_count

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [AssociationSignal](../classes/AssociationSignal.md) | An association signal from EHR, registry, or computational sources, optionall... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Integer](../types/Integer.md) |
| Domain Of | [AssociationSignal](../classes/AssociationSignal.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:disorder_b_count |
| native | dismech:disorder_b_count |




## LinkML Source

<details>
```yaml
name: disorder_b_count
description: Number of records/patients carrying disorder B in the source dataset
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: disorder_b_count
domain_of:
- AssociationSignal
range: integer

```
</details>