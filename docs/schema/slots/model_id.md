

# Slot: model_id 


_Identifier within the repository (e.g., Recon3D, BIOMD0000000123)_





URI: [dismech:slot/model_id](https://w3id.org/monarch-initiative/dismech/slot/model_id)
Alias: model_id

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComputationalModel](../classes/ComputationalModel.md) | A computational or in-silico model relevant to understanding disease mechanis... |  no  |






## Properties

* Range: [String](../types/String.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:model_id |
| native | dismech:model_id |




## LinkML Source

<details>
```yaml
name: model_id
description: Identifier within the repository (e.g., Recon3D, BIOMD0000000123)
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: model_id
domain_of:
- ComputationalModel
range: string

```
</details>