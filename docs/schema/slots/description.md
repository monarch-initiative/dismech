

# Slot: description 



URI: [dismech:slot/description](https://w3id.org/monarch-initiative/dismech/slot/description)
Alias: description

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Experiment](../classes/Experiment.md) | A structured experiment or protocol-level study design that can be proposed t... |  no  |
| [GeneticContext](../classes/GeneticContext.md) | A structured description of a genetic context that modifies phenotype frequen... |  no  |
| [FDASurrogateEndpointCollection](../classes/FDASurrogateEndpointCollection.md) | FDA surrogate endpoint table import preserving row-level source provenance |  no  |
| [AnimalModel](../classes/AnimalModel.md) |  |  no  |
| [AssayDescriptor](../classes/AssayDescriptor.md) | A descriptor for assays, bindable to OBI |  no  |
| [DifferentiatingMechanism](../classes/DifferentiatingMechanism.md) | A mechanism or feature that distinguishes a grouping member from its siblings... |  yes  |
| [Treatment](../classes/Treatment.md) |  |  no  |
| [ModelingConsideration](../classes/ModelingConsideration.md) |  |  no  |
| [Diagnosis](../classes/Diagnosis.md) |  |  no  |
| [ChemicalEntityDescriptor](../classes/ChemicalEntityDescriptor.md) | A descriptor for chemical entities, bindable to CHEBI |  no  |
| [ExperimentalModel](../classes/ExperimentalModel.md) | A disease-relevant non-animal experimental model system |  no  |
| [ComputationalModel](../classes/ComputationalModel.md) | A computational or in-silico model relevant to understanding disease mechanis... |  no  |
| [AnatomicalEntityDescriptor](../classes/AnatomicalEntityDescriptor.md) | A descriptor for anatomical locations, bindable to UBERON |  no  |
| [AgentLifeCycle](../classes/AgentLifeCycle.md) |  |  no  |
| [ConditionDescriptor](../classes/ConditionDescriptor.md) | A descriptor for a condition or disease, optionally bound to MONDO |  no  |
| [FoodDescriptor](../classes/FoodDescriptor.md) | A descriptor for foods, beverages, nutrients, minerals, and supplements, bind... |  no  |
| [CellularComponentDescriptor](../classes/CellularComponentDescriptor.md) | A descriptor for cellular components, bindable to GO cellular component |  no  |
| [UpstreamConditionHypothesis](../classes/UpstreamConditionHypothesis.md) | Hypothesized upstream condition that may explain both A and B |  no  |
| [ExperimentalPerturbation](../classes/ExperimentalPerturbation.md) | A structured perturbation, intervention, or exposure used in an experiment |  no  |
| [AgentLifeCycleStage](../classes/AgentLifeCycleStage.md) |  |  no  |
| [EnvironmentDescriptor](../classes/EnvironmentDescriptor.md) | A descriptor for environmental contexts/settings, bindable to ENVO |  no  |
| [ModelVariableDescriptor](../classes/ModelVariableDescriptor.md) | A descriptor mapping a model variable to an ontology term (LOINC, CHEBI, HP, ... |  no  |
| [BiologicalProcessDescriptor](../classes/BiologicalProcessDescriptor.md) | A descriptor for biological processes, bindable to Gene Ontology (GO) |  no  |
| [Grouping](../classes/Grouping.md) | An explicit, curated union of distinct Disease entries assembled below the le... |  no  |
| [PhenotypeDescriptor](../classes/PhenotypeDescriptor.md) | A descriptor for phenotypes, bindable to Human Phenotype Ontology (HP) |  no  |
| [BiomarkerReadout](../classes/BiomarkerReadout.md) | Links a biochemical biomarker to a pathograph node that it measures, reflects... |  no  |
| [ExposureDescriptor](../classes/ExposureDescriptor.md) | A descriptor for exposure events, bindable to ECTO or XCO |  no  |
| [MolecularFunctionDescriptor](../classes/MolecularFunctionDescriptor.md) | A descriptor for molecular functions, bindable to Gene Ontology (GO) |  no  |
| [Pathophysiology](../classes/Pathophysiology.md) |  |  no  |
| [HistopathologyFinding](../classes/HistopathologyFinding.md) | A histopathologic finding from microscopic examination of tissue |  yes  |
| [GroupingCriteria](../classes/GroupingCriteria.md) | The shared membership criteria for a grouping, pairing a human-readable descr... |  yes  |
| [ProteinComplexDescriptor](../classes/ProteinComplexDescriptor.md) | A descriptor for protein complexes that gene products participate in, bindabl... |  no  |
| [GeneProductDescriptor](../classes/GeneProductDescriptor.md) | A descriptor for gene products (proteins, fusion proteins, oncoproteins), bin... |  no  |
| [Stage](../classes/Stage.md) |  |  no  |
| [Transmission](../classes/Transmission.md) |  |  no  |
| [HostDescriptor](../classes/HostDescriptor.md) | A descriptor for hosts in an infectious agent life cycle |  no  |
| [Variant](../classes/Variant.md) | A genetic variant associated with a disease, including coding and non-coding ... |  no  |
| [FunctionalEffect](../classes/FunctionalEffect.md) | Describes the functional consequence of a genetic variant, including regulato... |  no  |
| [Environmental](../classes/Environmental.md) | An environmental factor, exposure, or context relevant to disease |  no  |
| [InheritanceDescriptor](../classes/InheritanceDescriptor.md) | A descriptor for inheritance patterns, bindable to HPO mode of inheritance te... |  no  |
| [CellTypeDescriptor](../classes/CellTypeDescriptor.md) | A descriptor for cell types, bindable to Cell Ontology (CL) |  no  |
| [ExternalAssertion](../classes/ExternalAssertion.md) | An externally curated assertion or registry record relevant to a disease or v... |  no  |
| [ImagingFindingDescriptor](../classes/ImagingFindingDescriptor.md) | A descriptor for an in-vivo imaging finding, bindable to the NCIT Imaging Fin... |  no  |
| [PhenotypeReadout](../classes/PhenotypeReadout.md) | Links an investigation-readout phenotype (an abnormal electrophysiology, func... |  no  |
| [EpidemiologyInfo](../classes/EpidemiologyInfo.md) |  |  no  |
| [DifferentialDiagnosis](../classes/DifferentialDiagnosis.md) | A disease or condition that presents similarly to the focal disease and must ... |  yes  |
| [ModelVariable](../classes/ModelVariable.md) | A variable in a computational model, identified by a human-readable name, wit... |  no  |
| [ModelMechanismLink](../classes/ModelMechanismLink.md) | Links an experimental model to a specific pathophysiology mechanism node, wit... |  yes  |
| [Subtype](../classes/Subtype.md) |  |  no  |
| [CriteriaSet](../classes/CriteriaSet.md) | A named criteria grouping within a definition |  no  |
| [CriteriaItem](../classes/CriteriaItem.md) | A criterion element (clinical feature, test result, imaging requirement) |  no  |
| [Disease](../classes/Disease.md) |  |  no  |
| [DiseaseDescriptor](../classes/DiseaseDescriptor.md) | A descriptor for the focal disease, bindable to MONDO |  no  |
| [Descriptor](../classes/Descriptor.md) | Base class for structured descriptors that allow a preferred term, optional d... |  yes  |
| [ImagingFinding](../classes/ImagingFinding.md) | A finding detected by in-vivo medical imaging (MRI, CT, PET, ultrasound, etc |  yes  |
| [Inheritance](../classes/Inheritance.md) |  |  no  |
| [TreatmentMechanismTarget](../classes/TreatmentMechanismTarget.md) | Links a treatment to a specific pathophysiology mechanism node it targets |  no  |
| [GeneDescriptor](../classes/GeneDescriptor.md) | A descriptor for genes, bindable to HGNC or other gene databases |  no  |
| [Definition](../classes/Definition.md) | A diagnostic or phenotype definition for the disease |  no  |
| [LogicalCriterion](../classes/LogicalCriterion.md) | A node in a nested boolean membership-criteria expression |  no  |
| [ComorbidityHypothesis](../classes/ComorbidityHypothesis.md) | Mechanistic hypothesis for a comorbidity association, with rich text and embe... |  no  |
| [TreatmentDescriptor](../classes/TreatmentDescriptor.md) | A descriptor for treatments/medical actions, bindable to NCIT clinical interv... |  no  |
| [RegimenDescriptor](../classes/RegimenDescriptor.md) | A descriptor for treatment regimens, bindable to NCIT |  no  |
| [CausalEdge](../classes/CausalEdge.md) | A reference to a downstream effect or consequence in a causal relationship |  no  |
| [Phenotype](../classes/Phenotype.md) |  |  no  |
| [BiomarkerDescriptor](../classes/BiomarkerDescriptor.md) | A descriptor for biomarkers, bindable to NCIT |  no  |
| [Mechanism](../classes/Mechanism.md) |  |  no  |
| [ExperimentalControl](../classes/ExperimentalControl.md) | A comparator or control condition for an experiment, such as an isogenic wild... |  no  |
| [GOEnrichment](../classes/GOEnrichment.md) | GO enrichment results for an association signal |  no  |
| [MechanisticHypothesis](../classes/MechanisticHypothesis.md) | Disease-level hypothesis metadata used to organize downstream causal edges in... |  no  |
| [SubtypeDescriptor](../classes/SubtypeDescriptor.md) | A descriptor for disease subtypes, bindable to MONDO disease terms or NCIT on... |  no  |
| [ProteinStructure](../classes/ProteinStructure.md) | A 3D protein structure from PDB or AlphaFold relevant to understanding a trea... |  no  |
| [TriggerDescriptor](../classes/TriggerDescriptor.md) | A descriptor for triggers/causes |  no  |
| [LifeCycleStageDescriptor](../classes/LifeCycleStageDescriptor.md) | A descriptor for parasite life cycle stages, bindable to OPL |  no  |
| [ExperimentalReadout](../classes/ExperimentalReadout.md) | A structured readout or outcome measured in an experiment |  no  |
| [ClinicalTrial](../classes/ClinicalTrial.md) | A clinical trial relevant to treatment or research of a disease |  yes  |
| [OrganismDescriptor](../classes/OrganismDescriptor.md) | A descriptor for organisms, bindable to NCBITaxon |  no  |
| [Assay](../classes/Assay.md) |  |  no  |
| [SurrogateEndpointCollection](../classes/SurrogateEndpointCollection.md) | A source-level collection of curated regulatory surrogate endpoint assertions |  no  |
| [HistopathologyFindingDescriptor](../classes/HistopathologyFindingDescriptor.md) | A descriptor for histopathologic findings, bindable to the NCIT Histopatholog... |  no  |
| [InfectiousAgent](../classes/InfectiousAgent.md) |  |  no  |
| [DietaryModification](../classes/DietaryModification.md) | A structured dietary addition, restriction, avoidance, or substitution used t... |  no  |
| [SampleTypeDescriptor](../classes/SampleTypeDescriptor.md) | A descriptor for biological sample types (tissue and/or cell type) |  no  |
| [Dataset](../classes/Dataset.md) | A reference to a publicly available omics or phenotype dataset |  yes  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [String](../types/String.md) |
| Domain Of | [Descriptor](../classes/Descriptor.md), [DietaryModification](../classes/DietaryModification.md), [GeneticContext](../classes/GeneticContext.md), [Dataset](../classes/Dataset.md), [ExperimentalModel](../classes/ExperimentalModel.md), [Experiment](../classes/Experiment.md), [ExperimentalPerturbation](../classes/ExperimentalPerturbation.md), [ExperimentalReadout](../classes/ExperimentalReadout.md), [ExperimentalControl](../classes/ExperimentalControl.md), [ClinicalTrial](../classes/ClinicalTrial.md), [ComputationalModel](../classes/ComputationalModel.md), [ModelVariable](../classes/ModelVariable.md), [DifferentialDiagnosis](../classes/DifferentialDiagnosis.md), [Subtype](../classes/Subtype.md), [CausalEdge](../classes/CausalEdge.md), [TreatmentMechanismTarget](../classes/TreatmentMechanismTarget.md), [ModelMechanismLink](../classes/ModelMechanismLink.md), [BiomarkerReadout](../classes/BiomarkerReadout.md), [PhenotypeReadout](../classes/PhenotypeReadout.md), [SurrogateEndpointCollection](../classes/SurrogateEndpointCollection.md), [ProteinStructure](../classes/ProteinStructure.md), [ExternalAssertion](../classes/ExternalAssertion.md), [EpidemiologyInfo](../classes/EpidemiologyInfo.md), [Pathophysiology](../classes/Pathophysiology.md), [Phenotype](../classes/Phenotype.md), [HistopathologyFinding](../classes/HistopathologyFinding.md), [ImagingFinding](../classes/ImagingFinding.md), [Environmental](../classes/Environmental.md), [Disease](../classes/Disease.md), [Stage](../classes/Stage.md), [AgentLifeCycle](../classes/AgentLifeCycle.md), [AgentLifeCycleStage](../classes/AgentLifeCycleStage.md), [AnimalModel](../classes/AnimalModel.md), [Treatment](../classes/Treatment.md), [InfectiousAgent](../classes/InfectiousAgent.md), [Transmission](../classes/Transmission.md), [Assay](../classes/Assay.md), [Diagnosis](../classes/Diagnosis.md), [Inheritance](../classes/Inheritance.md), [Variant](../classes/Variant.md), [FunctionalEffect](../classes/FunctionalEffect.md), [Mechanism](../classes/Mechanism.md), [ModelingConsideration](../classes/ModelingConsideration.md), [Definition](../classes/Definition.md), [CriteriaSet](../classes/CriteriaSet.md), [ConditionDescriptor](../classes/ConditionDescriptor.md), [GOEnrichment](../classes/GOEnrichment.md), [ComorbidityHypothesis](../classes/ComorbidityHypothesis.md), [UpstreamConditionHypothesis](../classes/UpstreamConditionHypothesis.md), [MechanisticHypothesis](../classes/MechanisticHypothesis.md), [Grouping](../classes/Grouping.md), [GroupingCriteria](../classes/GroupingCriteria.md), [LogicalCriterion](../classes/LogicalCriterion.md), [DifferentiatingMechanism](../classes/DifferentiatingMechanism.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:description |
| native | dismech:description |




## LinkML Source

<details>
```yaml
name: description
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: description
domain_of:
- Descriptor
- DietaryModification
- GeneticContext
- Dataset
- ExperimentalModel
- Experiment
- ExperimentalPerturbation
- ExperimentalReadout
- ExperimentalControl
- ClinicalTrial
- ComputationalModel
- ModelVariable
- DifferentialDiagnosis
- Subtype
- CausalEdge
- TreatmentMechanismTarget
- ModelMechanismLink
- BiomarkerReadout
- PhenotypeReadout
- SurrogateEndpointCollection
- ProteinStructure
- ExternalAssertion
- EpidemiologyInfo
- Pathophysiology
- Phenotype
- HistopathologyFinding
- ImagingFinding
- Environmental
- Disease
- Stage
- AgentLifeCycle
- AgentLifeCycleStage
- AnimalModel
- Treatment
- InfectiousAgent
- Transmission
- Assay
- Diagnosis
- Inheritance
- Variant
- FunctionalEffect
- Mechanism
- ModelingConsideration
- Definition
- CriteriaSet
- ConditionDescriptor
- GOEnrichment
- ComorbidityHypothesis
- UpstreamConditionHypothesis
- MechanisticHypothesis
- Grouping
- GroupingCriteria
- LogicalCriterion
- DifferentiatingMechanism
range: string

```
</details>