

# Slot: sample_types 


_Types of biological samples in the dataset_





URI: [dismech:slot/sample_types](https://w3id.org/monarch-initiative/dismech/slot/sample_types)
Alias: sample_types

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Dataset](../classes/Dataset.md) | A reference to a publicly available omics or phenotype dataset |  no  |






## Properties

* Range: [SampleTypeDescriptor](../classes/SampleTypeDescriptor.md)

* Multivalued: True




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:sample_types |
| native | dismech:sample_types |




## LinkML Source

<details>
```yaml
name: sample_types
description: Types of biological samples in the dataset
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: sample_types
domain_of:
- Dataset
range: SampleTypeDescriptor
multivalued: true
inlined: true
inlined_as_list: true

```
</details>