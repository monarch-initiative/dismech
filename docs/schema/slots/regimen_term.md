

# Slot: regimen_term 


_The NCIT term for this treatment regimen_





URI: [dismech:slot/regimen_term](https://w3id.org/monarch-initiative/dismech/slot/regimen_term)
Alias: regimen_term

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Treatment](../classes/Treatment.md) |  |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [RegimenDescriptor](../classes/RegimenDescriptor.md) |
| Domain Of | [Treatment](../classes/Treatment.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:regimen_term |
| native | dismech:regimen_term |




## LinkML Source

<details>
```yaml
name: regimen_term
description: The NCIT term for this treatment regimen
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: regimen_term
domain_of:
- Treatment
range: RegimenDescriptor
inlined: true

```
</details>