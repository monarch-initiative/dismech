

# Slot: exposure_term 


_The ECTO/XCO term for this exposure event_





URI: [dismech:slot/exposure_term](https://w3id.org/monarch-initiative/dismech/slot/exposure_term)
Alias: exposure_term

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Environmental](../classes/Environmental.md) | An environmental factor, exposure, or context relevant to disease |  no  |






## Properties

* Range: [ExposureDescriptor](../classes/ExposureDescriptor.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:exposure_term |
| native | dismech:exposure_term |




## LinkML Source

<details>
```yaml
name: exposure_term
description: The ECTO/XCO term for this exposure event
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: exposure_term
domain_of:
- Environmental
range: ExposureDescriptor
inlined: true

```
</details>