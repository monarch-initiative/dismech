

# Slot: onset_category 


_HPO onset category (e.g., CHILDHOOD, NEONATAL). Use when an approximate developmental stage is known._





URI: [dismech:slot/onset_category](https://w3id.org/monarch-initiative/dismech/slot/onset_category)
Alias: onset_category

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [OnsetDescriptor](../classes/OnsetDescriptor.md) | Structured description of age of onset |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [OnsetEnum](../enums/OnsetEnum.md) |
| Domain Of | [OnsetDescriptor](../classes/OnsetDescriptor.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:onset_category |
| native | dismech:onset_category |




## LinkML Source

<details>
```yaml
name: onset_category
description: HPO onset category (e.g., CHILDHOOD, NEONATAL). Use when an approximate
  developmental stage is known.
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: onset_category
domain_of:
- OnsetDescriptor
range: OnsetEnum

```
</details>