

# Slot: components 


_Component conditions that make up a composite descriptor_





URI: [dismech:slot/components](https://w3id.org/monarch-initiative/dismech/slot/components)
Alias: components

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ConditionDescriptor](../classes/ConditionDescriptor.md) | A descriptor for a condition or disease, optionally bound to MONDO |  no  |






## Properties

* Range: [ConditionDescriptor](../classes/ConditionDescriptor.md)

* Multivalued: True




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:components |
| native | dismech:components |




## LinkML Source

<details>
```yaml
name: components
description: Component conditions that make up a composite descriptor
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: components
domain_of:
- ConditionDescriptor
range: ConditionDescriptor
multivalued: true
inlined: true
inlined_as_list: true

```
</details>