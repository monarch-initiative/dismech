

# Slot: curation_status 


_Curation workflow status_





URI: [dismech:slot/curation_status](https://w3id.org/monarch-initiative/dismech/slot/curation_status)
Alias: curation_status

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComorbidityAssociation](../classes/ComorbidityAssociation.md) | An association between two conditions, including directionality, evidence, an... |  no  |






## Properties

* Range: [CurationStatusEnum](../enums/CurationStatusEnum.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:curation_status |
| native | dismech:curation_status |




## LinkML Source

<details>
```yaml
name: curation_status
description: Curation workflow status
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: curation_status
domain_of:
- ComorbidityAssociation
range: CurationStatusEnum

```
</details>