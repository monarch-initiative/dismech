

# Slot: life_cycle_stage_term 


_The OPL term for this agent life cycle stage_





URI: [dismech:slot/life_cycle_stage_term](https://w3id.org/monarch-initiative/dismech/slot/life_cycle_stage_term)
Alias: life_cycle_stage_term

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [AgentLifeCycleStage](../classes/AgentLifeCycleStage.md) |  |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [LifeCycleStageDescriptor](../classes/LifeCycleStageDescriptor.md) |
| Domain Of | [AgentLifeCycleStage](../classes/AgentLifeCycleStage.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:life_cycle_stage_term |
| native | dismech:life_cycle_stage_term |




## LinkML Source

<details>
```yaml
name: life_cycle_stage_term
description: The OPL term for this agent life cycle stage
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: life_cycle_stage_term
domain_of:
- AgentLifeCycleStage
range: LifeCycleStageDescriptor
inlined: true

```
</details>