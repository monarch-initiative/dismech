

# Slot: mechanistic_category 


_Mechanistic/pathway-based disease classification_





URI: [dismech:slot/mechanistic_category](https://w3id.org/monarch-initiative/dismech/slot/mechanistic_category)
Alias: mechanistic_category

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [DiseaseClassifications](../classes/DiseaseClassifications.md) | Container for all classification assignments for a disease |  no  |






## Properties

* Range: [MechanisticNosologyAssignment](../classes/MechanisticNosologyAssignment.md)

* Multivalued: True




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:mechanistic_category |
| native | dismech:mechanistic_category |




## LinkML Source

<details>
```yaml
name: mechanistic_category
description: Mechanistic/pathway-based disease classification
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: mechanistic_category
domain_of:
- DiseaseClassifications
range: MechanisticNosologyAssignment
multivalued: true
inlined: true

```
</details>