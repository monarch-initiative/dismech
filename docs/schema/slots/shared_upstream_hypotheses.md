

# Slot: shared_upstream_hypotheses 


_Suspected upstream conditions that may explain both A and B_





URI: [dismech:slot/shared_upstream_hypotheses](https://w3id.org/monarch-initiative/dismech/slot/shared_upstream_hypotheses)
Alias: shared_upstream_hypotheses

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComorbidityAssociation](../classes/ComorbidityAssociation.md) | An association between two conditions, including directionality, evidence, an... |  no  |






## Properties

* Range: [UpstreamConditionHypothesis](../classes/UpstreamConditionHypothesis.md)

* Multivalued: True




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:shared_upstream_hypotheses |
| native | dismech:shared_upstream_hypotheses |




## LinkML Source

<details>
```yaml
name: shared_upstream_hypotheses
description: Suspected upstream conditions that may explain both A and B
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: shared_upstream_hypotheses
domain_of:
- ComorbidityAssociation
range: UpstreamConditionHypothesis
multivalued: true
inlined: true
inlined_as_list: true

```
</details>