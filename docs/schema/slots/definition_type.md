

# Slot: definition_type 


_The type of definition or criteria set_





URI: [dismech:slot/definition_type](https://w3id.org/monarch-initiative/dismech/slot/definition_type)
Alias: definition_type

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Definition](../classes/Definition.md) | A diagnostic or phenotype definition for the disease |  yes  |






## Properties

* Range: [DefinitionTypeEnum](../enums/DefinitionTypeEnum.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:definition_type |
| native | dismech:definition_type |




## LinkML Source

<details>
```yaml
name: definition_type
description: The type of definition or criteria set
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: definition_type
domain_of:
- Definition
range: DefinitionTypeEnum

```
</details>