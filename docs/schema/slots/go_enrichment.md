

# Slot: go_enrichment 


_GO enrichment results associated with a signal_





URI: [dismech:slot/go_enrichment](https://w3id.org/monarch-initiative/dismech/slot/go_enrichment)
Alias: go_enrichment

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [AssociationSignal](../classes/AssociationSignal.md) | An association signal from EHR, registry, or computational sources, optionall... |  no  |






## Properties

* Range: [GOEnrichment](../classes/GOEnrichment.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:go_enrichment |
| native | dismech:go_enrichment |




## LinkML Source

<details>
```yaml
name: go_enrichment
description: GO enrichment results associated with a signal
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: go_enrichment
domain_of:
- AssociationSignal
range: GOEnrichment
inlined: true

```
</details>