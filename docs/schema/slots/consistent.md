

# Slot: consistent 


_Consistency status for a mapping relative to a reference source_





URI: [dismech:slot/consistent](https://w3id.org/monarch-initiative/dismech/slot/consistent)
Alias: consistent

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [MappingConsistency](../classes/MappingConsistency.md) | Consistency assertion for a mapping relative to another source |  yes  |






## Properties

* Range: [MappingConsistencyEnum](../enums/MappingConsistencyEnum.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:consistent |
| native | dismech:consistent |




## LinkML Source

<details>
```yaml
name: consistent
description: Consistency status for a mapping relative to a reference source
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: consistent
domain_of:
- MappingConsistency
range: MappingConsistencyEnum

```
</details>