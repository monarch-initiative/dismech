

# Slot: repository_url 


_URL to model repository (GitHub, BiGG, VMH, BioModels)_





URI: [dismech:slot/repository_url](https://w3id.org/monarch-initiative/dismech/slot/repository_url)
Alias: repository_url

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComputationalModel](../classes/ComputationalModel.md) | A computational or in-silico model relevant to understanding disease mechanis... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Uri](../types/Uri.md) |
| Domain Of | [ComputationalModel](../classes/ComputationalModel.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:repository_url |
| native | dismech:repository_url |




## LinkML Source

<details>
```yaml
name: repository_url
description: URL to model repository (GitHub, BiGG, VMH, BioModels)
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: repository_url
domain_of:
- ComputationalModel
range: uri

```
</details>