

# Slot: base_model 


_Parent/base model this is derived from (e.g., Recon3D, Harvey 1.0)_





URI: [dismech:slot/base_model](https://w3id.org/monarch-initiative/dismech/slot/base_model)
Alias: base_model

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComputationalModel](../classes/ComputationalModel.md) | A computational or in-silico model relevant to understanding disease mechanis... |  no  |






## Properties

* Range: [String](../types/String.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:base_model |
| native | dismech:base_model |




## LinkML Source

<details>
```yaml
name: base_model
description: Parent/base model this is derived from (e.g., Recon3D, Harvey 1.0)
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: base_model
domain_of:
- ComputationalModel
range: string

```
</details>