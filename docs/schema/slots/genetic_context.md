

# Slot: genetic_context 


_The genetic context under which this qualification applies. May specify genes, mutation types, zygosity, complementation groups, or complex genotypes._





URI: [dismech:slot/genetic_context](https://w3id.org/monarch-initiative/dismech/slot/genetic_context)
Alias: genetic_context

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [PhenotypeContext](../classes/PhenotypeContext.md) | A context-specific annotation qualifying how a phenotype manifests under part... |  no  |






## Properties

* Range: [GeneticContext](../classes/GeneticContext.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:genetic_context |
| native | dismech:genetic_context |




## LinkML Source

<details>
```yaml
name: genetic_context
description: The genetic context under which this qualification applies. May specify
  genes, mutation types, zygosity, complementation groups, or complex genotypes.
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: genetic_context
domain_of:
- PhenotypeContext
range: GeneticContext
inlined: true

```
</details>