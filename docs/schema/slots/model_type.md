

# Slot: model_type 


_Type of computational model_





URI: [dismech:slot/model_type](https://w3id.org/monarch-initiative/dismech/slot/model_type)
Alias: model_type

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComputationalModel](../classes/ComputationalModel.md) | A computational or in-silico model relevant to understanding disease mechanis... |  no  |






## Properties

* Range: [ComputationalModelTypeEnum](../enums/ComputationalModelTypeEnum.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:model_type |
| native | dismech:model_type |




## LinkML Source

<details>
```yaml
name: model_type
description: Type of computational model
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: model_type
domain_of:
- ComputationalModel
range: ComputationalModelTypeEnum

```
</details>