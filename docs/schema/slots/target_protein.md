

# Slot: target_protein 


_Name of the protein target in the structure_





URI: [dismech:slot/target_protein](https://w3id.org/monarch-initiative/dismech/slot/target_protein)
Alias: target_protein

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ProteinStructure](../classes/ProteinStructure.md) | A 3D protein structure from PDB or AlphaFold relevant to understanding a trea... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [String](../types/String.md) |
| Domain Of | [ProteinStructure](../classes/ProteinStructure.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| Owner | [ProteinStructure](../classes/ProteinStructure.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:target_protein |
| native | dismech:target_protein |




## LinkML Source

<details>
```yaml
name: target_protein
description: Name of the protein target in the structure
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: target_protein
owner: ProteinStructure
domain_of:
- ProteinStructure
range: string

```
</details>