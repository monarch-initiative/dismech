

# Slot: infectious_agent_term 


_The NCBITaxon term for this infectious agent_





URI: [dismech:slot/infectious_agent_term](https://w3id.org/monarch-initiative/dismech/slot/infectious_agent_term)
Alias: infectious_agent_term

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [InfectiousAgent](../classes/InfectiousAgent.md) |  |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [OrganismDescriptor](../classes/OrganismDescriptor.md) |
| Domain Of | [InfectiousAgent](../classes/InfectiousAgent.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:infectious_agent_term |
| native | dismech:infectious_agent_term |




## LinkML Source

<details>
```yaml
name: infectious_agent_term
description: The NCBITaxon term for this infectious agent
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: infectious_agent_term
domain_of:
- InfectiousAgent
range: OrganismDescriptor
inlined: true

```
</details>