

# Slot: exposures 


_Environmental exposures studied in the dataset_





URI: [dismech:slot/exposures](https://w3id.org/monarch-initiative/dismech/slot/exposures)
Alias: exposures

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Dataset](../classes/Dataset.md) | A reference to a publicly available omics or phenotype dataset |  no  |






## Properties

* Range: [ExposureDescriptor](../classes/ExposureDescriptor.md)

* Multivalued: True




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:exposures |
| native | dismech:exposures |




## LinkML Source

<details>
```yaml
name: exposures
description: Environmental exposures studied in the dataset
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: exposures
domain_of:
- Dataset
range: ExposureDescriptor
multivalued: true
inlined: true
inlined_as_list: true

```
</details>