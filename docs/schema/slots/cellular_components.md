

# Slot: cellular_components 



URI: [dismech:slot/cellular_components](https://w3id.org/monarch-initiative/dismech/slot/cellular_components)
Alias: cellular_components

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Pathophysiology](../classes/Pathophysiology.md) |  |  no  |






## Properties

* Range: [CellularComponentDescriptor](../classes/CellularComponentDescriptor.md)

* Multivalued: True





## Examples

| Value |
| --- |
| [{preferred_term: Peroxisome}] |

## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:cellular_components |
| native | dismech:cellular_components |




## LinkML Source

<details>
```yaml
name: cellular_components
examples:
- value: '[{preferred_term: Peroxisome}]'
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: cellular_components
domain_of:
- Pathophysiology
range: CellularComponentDescriptor
multivalued: true
inlined: true
inlined_as_list: true

```
</details>