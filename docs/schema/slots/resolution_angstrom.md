

# Slot: resolution_angstrom 


_Structure resolution in angstroms (for experimental structures)_





URI: [dismech:slot/resolution_angstrom](https://w3id.org/monarch-initiative/dismech/slot/resolution_angstrom)
Alias: resolution_angstrom

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ProteinStructure](../classes/ProteinStructure.md) | A 3D protein structure from PDB or AlphaFold relevant to understanding a trea... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Float](../types/Float.md) |
| Domain Of | [ProteinStructure](../classes/ProteinStructure.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| Owner | [ProteinStructure](../classes/ProteinStructure.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:resolution_angstrom |
| native | dismech:resolution_angstrom |




## LinkML Source

<details>
```yaml
name: resolution_angstrom
description: Structure resolution in angstroms (for experimental structures)
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: resolution_angstrom
owner: ProteinStructure
domain_of:
- ProteinStructure
range: float

```
</details>