

# Slot: environment_context 


_The ENVO term for the environmental context/setting_





URI: [dismech:slot/environment_context](https://w3id.org/monarch-initiative/dismech/slot/environment_context)
Alias: environment_context

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [Environmental](../classes/Environmental.md) | An environmental factor, exposure, or context relevant to disease |  no  |






## Properties

* Range: [EnvironmentDescriptor](../classes/EnvironmentDescriptor.md)




## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:environment_context |
| native | dismech:environment_context |




## LinkML Source

<details>
```yaml
name: environment_context
description: The ENVO term for the environmental context/setting
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: environment_context
domain_of:
- Environmental
range: EnvironmentDescriptor
inlined: true

```
</details>