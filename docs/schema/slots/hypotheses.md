

# Slot: hypotheses 


_Mechanistic or causal hypotheses about the association_





URI: [dismech:slot/hypotheses](https://w3id.org/monarch-initiative/dismech/slot/hypotheses)
Alias: hypotheses

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [ComorbidityAssociation](../classes/ComorbidityAssociation.md) | An association between two conditions, including directionality, evidence, an... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [ComorbidityHypothesis](../classes/ComorbidityHypothesis.md) |
| Domain Of | [ComorbidityAssociation](../classes/ComorbidityAssociation.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
| Multivalued | Yes |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/monarch-initiative/dismech




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | dismech:hypotheses |
| native | dismech:hypotheses |




## LinkML Source

<details>
```yaml
name: hypotheses
description: Mechanistic or causal hypotheses about the association
from_schema: https://w3id.org/monarch-initiative/dismech
rank: 1000
alias: hypotheses
domain_of:
- ComorbidityAssociation
range: ComorbidityHypothesis
multivalued: true
inlined: true
inlined_as_list: true

```
</details>