...
**Post-publication note (2026-09-16).** The ladder in §4 was ratified as design
decisions **§3e** — not §3b, which was taken by the time it landed — with three
corrections that the discussion on #10115 produced after this report was
written, all recorded there rather than edited into the text below:

1. §2.3 and §5 item 4 are **withdrawn** for `Spotted_Fever_Rickettsiosis`. Its
   RMSF and boutonneuse fever rows are *pointer* subtypes doing what §3a L4
   prescribes for a promoted stratum, not a second copy of the disease; the
   pointer is now machine-readable (`Subtype.curated_in`). The finding stands
   for `Viral_Hemorrhagic_Fever`.
2. §5 item 6's `pathotype:` slot is **not added**. A pathotype is a disease-level
   `has_subtypes` row with `classification: pathotype`; agent-level strata are
   taxonomic and bind NCBITaxon once PR #10353 lands.
3. The case catalog proposed in the follow-up discussion is not created; the
   checker `just check-granularity` is the computed half, and a decision about
   a lump is recorded in the entry itself.
...
- [#10115](https://github.com/monarch-initiative/dismech/issues/10115) — the microbial ladder and its work items (§4, §5 items 1–11).
- [#10116](https://github.com/monarch-initiative/dismech/issues/10116) — the cross-KB defects this review surfaced (§5 items 12–14).
- [#10113](https://github.com/monarch-initiative/dismech/issues/10113) — shared `disease_term` values without a `skos:narrowMatch` anchor; the same defect as §2.7's over-broad anchoring, reached from the mapping side.
