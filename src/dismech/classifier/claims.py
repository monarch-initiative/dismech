...
def class_at(path: str) -> str:
    ...
    i = 0
    while i < len(parts):
        slots = {s.name: s for s in schema().class_induced_slots(current)}
...
def local_context(node: dict, class_name: str) -> dict:
    ...
    for slot in schema().class_induced_slots(class_name):
        ...
        if any(
            r in schema().all_classes() and "evidence" in schema().class_slots(r)
            for r in ranges
        ):
...
def extract_claim(
    ...
    if "evidence" not in schema().class_slots(assertion_type):
