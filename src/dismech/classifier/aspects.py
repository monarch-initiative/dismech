...
def aspect_output_schema(claim, fields=DEFAULT_FIELDS):
    ...
    fields=None assesses every leaf field, keeping term ID/label pairs and
    scalar lists together, plus every inherited context binding.
    """
        ...
        if isinstance(value, dict):
            for key, item in value.items():
                ...
                selected = (
                    (
                        key == "term"
                        or not isinstance(item, (dict, list))
                        or (
                            isinstance(item, list)
                            and all(not isinstance(v, (dict, list)) for v in item)
                        )
                    )
                    if fields is None
                    else key in fields
                )
                if selected and item is not None and item != [] and item != {}:
    ...
    if fields is None:
        for index, binding in enumerate(state["about"].get("context", [])):
            add(
                f"/about/context/{index}/value",
                "Assess whether the finding applies under this inherited ancestor "
                "or subtype scope. The binding describes the claim, not evidence.",
            )
    return {
        "type": "object",
