---
reference_id: "STRCHIVE:CCHS_PHOX2B"
title: "PHOX2B — Congenital central hypoventilation syndrome"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:CCHS_PHOX2B  PHOX2B — Congenital central hypoventilation syndrome

**STRCHIVE:CCHS_PHOX2B** — STRchive tandem-repeat disease locus `CCHS_PHOX2B` (PHOX2B, Congenital central hypoventilation syndrome).

## Locus

- STRchive ID: CCHS_PHOX2B
- Gene: PHOX2B
- Gene strand: -
- Disease: Congenital central hypoventilation syndrome
- Disease abbreviation: CCHS
- Location type: Coding
- Location in gene: Exon 3
- Inheritance: AD (Autosomal dominant)
- Mechanism: LoF/GoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | GCN |
| Pathogenic motif (reference orientation) | GCN |
| Pathogenic motif (gene orientation) | CNG |
| Motif length (bp) | 3 |
| Locus structure | (GCN)* |
| Reference copies | 15.7 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 9 | 20 |
| Intermediate | 21 | 25 |
| Pathogenic | 26 | 33 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr4 | 41745972 | 41746032 |
| hg19 | chr4 | 41747989 | 41748049 |
| T2T-chm13 | chr4 | 41719745 | 41719805 |

## Disease description

A rare disease due to a severely impaired central autonomic control of breathing and dysfunction of the autonomous nervous system. The incidence is estimated to be at 1 of 200 000 livebirths. A heterozygous mutation of PHOX-2B gene is found in 90% of the patients. Association with a Hirschsprung's disease is observed in 16% of the cases (adapted from Mondo) [@mondo:0800026].

## Mechanism

Polyalanine expansion leading to loss or gain of function, dependent on altered protein product [@pmid:38467784; @genereviews:NBK1427]. Correlation between length and reduced transcriptional activity [@pmid:15888479].

## Age of onset

Typical: 0-2 [@genereviews:NBK1427; @pmid:15121777]; Range: 0-36 [@pmid:16873766].

## Prevalence

- Details: Incidence is 1:148000-200000 births (Estimated, may include mild/undiagnosed or be overestimated globally) [@genereviews:NBK1427]. Rare, but reported worldwide [@pmid:15121777].

## Details

Alleles of 24 repeats (and sometimes 25 repeats) correspond to delayed disease onset and/or milder phenotype; alleles above benign range (9-20 repeats) and below the pathogenic range (26-33 repeats) have uncertain significance [@genereviews:NBK1427].

## Tags

- Locus tags: length_affects_onset, length_affects_severity

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0800026 |
| OMIM | OMIM:209880 |
| Orphanet | ORPHA:661 |
| MedGen | 1794285 |
| GARD | 8535 |
| MalaCards | CNT119 |
| GeneReviews | NBK1427 |
| gnomAD | PHOX2B |
| STRipy | PHOX2B |
| TR-Atlas | TR42548 |

## References

- genereviews:NBK1427
- pmid:15121777
- pmid:16873766
- pmid:38467784
- pmid:15888479
- pmid:12640453
- mondo:0800026

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
