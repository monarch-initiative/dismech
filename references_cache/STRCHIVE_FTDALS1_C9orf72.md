---
reference_id: "STRCHIVE:FTDALS1_C9orf72"
title: "C9orf72 — Frontotemporal dementia (FTD) and/or amyotrophic lateral sclerosis (ALS)"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:FTDALS1_C9orf72  C9orf72 — Frontotemporal dementia (FTD) and/or amyotrophic lateral sclerosis (ALS)

**STRCHIVE:FTDALS1_C9orf72** — STRchive tandem-repeat disease locus `FTDALS1_C9orf72` (C9orf72, Frontotemporal dementia (FTD) and/or amyotrophic lateral sclerosis (ALS)).

## Locus

- STRchive ID: FTDALS1_C9orf72
- Gene: C9orf72
- Gene strand: -
- Disease: Frontotemporal dementia (FTD) and/or amyotrophic lateral sclerosis (ALS)
- Disease abbreviation: FTDALS1
- Location type: 5' UTR/Intronic
- Location in gene: Intron 1 or 5' UTR depending on transcript
- Inheritance: AD (Autosomal dominant)
- Mechanism: Ambiguous

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | GGCCCC |
| Pathogenic motif (reference orientation) | GGCCCC |
| Pathogenic motif (gene orientation) | CCGGGG |
| Motif length (bp) | 6 |
| Locus structure | (GGCCCC)* |
| Reference copies | 10.8 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 2 | 23 |
| Intermediate | 24 | 60 |
| Pathogenic | 251 | 4088 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr9 | 27573484 | 27573546 |
| hg19 | chr9 | 27573482 | 27573544 |
| T2T-chm13 | chr9 | 27584063 | 27584155 |

## Disease description

Pure frontotemporal dementia, pure amyotrophic lateral sclerosis or combination of the two [@pmid:39349043].

## Mechanism

The HRE forms DNA and RNA G-quadruplexes with distinct structures and promotes RNA/DNA hybrids (R-loops). The structural polymorphism causes a repeat length-dependent accumulation of transcripts aborted in the HRE region [@omim:105500]. Addiitonal mechanisms theorized include protein loss of function and RNA gain of function [@pmid:37847372].

## Age of onset

Typical: 50-64; Range: 20-91 [@genereviews:NBK268647].

## Prevalence

- Details: The expansion of a hexanucleotide repeat GGGGCC in C9orf72 is the most common known cause of ALS accounting for ~ 40% familial cases and ~ 7% sporadic cases in the European population; overall ALS incidence is 1-2/100,000 person-years, point prevalence is 3-5/100,000 (Europe/US); lifetime risk is 1 in 300 [@pmid:31315673]. Related individuals to patients with C9orf72-ALS appear at an increased risk of disease regardless of carrier status [@pmid:38149039; @pmid:39315390]. C9orf72-FTD is estimated to be 0.04-134:100,000 [@genereviews:NBK268647], and by our estimates 0.65-1.56/100,000 for C9orf72-ALS. The expansion has been found across ethnicities/ancestries, with population-dependent prevalence, highest in those with northern European ancestry [@genereviews:NBK268647].

## Details

FTD and ALS form a clinical spectrum [@pmid:37388914; @pmid:22406228]. The clinical ranges of the C9orf72 locus remain ambiguous [@stripy:C9ORF72]: most healthy controls have alleles up to 24 repeats [@pmid:28319737] yet 24-30 repeats are associated with ALS [@pmid:31315673] and while 60 repeats is frequently used as a threshold for uncertain alleles, the exact threshold of pathogenicity remains unclear [@genereviews:NBK268647; @pmid:38099605]. Repeat of 80 motifs and lower appear to have delayed onset for any phenotype [@pmid:28319737]. >250 repeats are associated with a full FTD/ALS disease state [@pmid:31048495], but pathogenic alleles can range from 30 to more than 4000 repeats [@pmid:38099605; @pmid:39709476]. Penetrance appears to also be age-dependent, with environmental factors and specific phenotypes associated with sex and age at onset [@pmid:28522837]. Methylation appears to increase with expansion length and age [@pmid:39709476].

## Tags

- Locus tags: length_affects_phenotype, length_affects_onset
- Disease tags: phenotypic_spectrum

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0007105 |
| OMIM | OMIM:105500 |
| Orphanet | ORPHA:275872 |
| MedGen | 1830423 |
| GARD | 18396 |
| MalaCards | FRN044 |
| GeneReviews | NBK268647 |
| gnomAD | C9ORF72 |
| STRipy | C9ORF72 |
| TR-Atlas | TR92417 |

## References

- genereviews:NBK268647
- omim:105500
- pmid:37847372
- pmid:37388914
- pmid:22406228
- stripy:C9ORF72
- pmid:28319737
- pmid:31315673
- pmid:38099605
- pmid:31048495
- pmid:28522837
- pmid:38149039
- pmid:39315390
- pmid:21944778
- pmid:39349043

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
