---
reference_id: "STRCHIVE:DMD_DMD"
title: "DMD — Duchenne muscular dystrophy"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:DMD_DMD  DMD — Duchenne muscular dystrophy

**STRCHIVE:DMD_DMD** — STRchive tandem-repeat disease locus `DMD_DMD` (DMD, Duchenne muscular dystrophy).

## Locus

- STRchive ID: DMD_DMD
- Gene: DMD
- Gene strand: -
- Disease: Duchenne muscular dystrophy
- Disease abbreviation: DMD
- Location type: Intronic
- Location in gene: Intron 62
- Inheritance: XR (X-linked recessive)
- Mechanism: LoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | TTC |
| Pathogenic motif (reference orientation) | TTC |
| Pathogenic motif (gene orientation) | AAG |
| Motif length (bp) | 3 |
| Locus structure | (TTC)* |
| Reference copies | 16.7 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 16 | 33 |
| Intermediate | - | - |
| Pathogenic | 59 | 82 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chrX | 31284557 | 31284605 |
| hg19 | chrX | 31302674 | 31302722 |
| T2T-chm13 | chrX | 30882695 | 30882743 |

## Disease description

Duchenne muscular dystrophy (DMD) is a neuromuscular disease characterized by rapidly progressive muscle weakness and wasting due to degeneration of skeletal, smooth and cardiac muscle [@mondo:0010679].

## Mechanism

Functional defect in dystrophin/dystroglycan [@pmid:16969582].

## Age of onset

Typical: 6-7 (usual disease is 0-3) [@pmid:27417533].

## Prevalence

- Prevalence: 4.8/100000
- Details: Believed to be 0 for disease specific to STR expansion. 1/3500-4700 male births (incidence) for overall DMD (one of the most common and severe congenital myopathies) [@genereviews:NBK1119]. 4.8/100,000 prevalence [@pmid:35168641]. DMD repeat locus expansion only identified in one Greek family [@pmid:27417533].

## Details

There is conflicting evidence for the association between this repeat expansion and Duchenne muscular dystrophy. The association was reported in a single family, from which the benign and pathogenic ranges were inferred from affected and unaffected family members [@pmid:27417533]. The population frequency of the proposed pathogenic allele is much higher than expected for a highly penetrant early-onset condition.

## Tags

- Locus tags: conflicting_evidence, sparse_evidence

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0010679 |
| OMIM | OMIM:310200 |
| Orphanet | ORPHA:98896 |
| MedGen | 3925 |
| GARD | 6291 |
| MalaCards | MSC157 |
| GeneReviews | NBK535148 |
| gnomAD | DMD |
| STRipy | DMD |
| TR-Atlas | TR167703 |

## References

- pmid:27417533
- pmid:16969582
- genereviews:NBK1119
- pmid:35168641
- mondo:0010679

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
