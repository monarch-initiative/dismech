---
reference_id: "STRCHIVE:HMNR7_VWA1"
title: "VWA1 — Neuronopathy, distal hereditary motor, autosomal recessive 7"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:HMNR7_VWA1  VWA1 — Neuronopathy, distal hereditary motor, autosomal recessive 7

**STRCHIVE:HMNR7_VWA1** — STRchive tandem-repeat disease locus `HMNR7_VWA1` (VWA1, Neuronopathy, distal hereditary motor, autosomal recessive 7).

## Locus

- STRchive ID: HMNR7_VWA1
- Gene: VWA1
- Gene strand: +
- Disease: Neuronopathy, distal hereditary motor, autosomal recessive 7
- Disease abbreviation: HMNR7
- Location type: Coding
- Location in gene: Exon 1
- Inheritance: AR (Autosomal recessive)
- Mechanism: LoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | GGCGCGGAGC |
| Pathogenic motif (reference orientation) | GGCGCGGAGC |
| Pathogenic motif (gene orientation) | AGCGGCGCGG |
| Motif length (bp) | 10 |
| Locus structure | (GGCGCGGAGC)* |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 2 | 2 |
| Intermediate | - | - |
| Pathogenic | 1 | 3 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr1 | 1435798 | 1435818 |
| hg19 | chr1 | 1371178 | 1371198 |
| T2T-chm13 | chr1 | 870158 | 870178 |

## Disease description

Autosomal recessive distal hereditary motor neuronopathy-7 (HMNR7) is characterized by onset of lower leg weakness in the first decade. Affected individuals have difficulty climbing stairs and problems standing on the heels... Most patients have foot deformities, and some may have leg muscle atrophy. The disorder is slowly progressive and often involves the upper limbs [@omim:619216].

## Mechanism

Loss of function [@pmid:38467784].

## Age of onset

Typical: 1-3 [@pmid:33559681]; Range: 0-10 [@omim:619216].

## Prevalence

- Details: Biallelic variants found in 0.01% of 100 KGP participants; enriched in those with motor disease. 80% of VWA1 pathogenic variants are expansions/contractions [@genereviews:NBK535148]. The repeat expansion was found in several ethnicities; founder mutation 7,000 years prior [@pmid:33559681].

## Details

Any deviation from 2 motifs is thought to be pathogenic [@genereviews:NBK535148].

## Tags

- Locus tags: contraction

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0030977 |
| OMIM | OMIM:619216 |
| Orphanet | ORPHA:314485 |
| MedGen | 1786836 |
| GARD | 18444 |
| MalaCards | NRN074 |
| GeneReviews | NBK535148 |
| gnomAD | VWA1 |
| TR-Atlas | TR32 |

## References

- pmid:33559681
- omim:619216
- pmid:38467784
- genereviews:NBK535148

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
