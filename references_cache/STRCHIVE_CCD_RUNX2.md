---
reference_id: "STRCHIVE:CCD_RUNX2"
title: "RUNX2 — Cleidocranial dysplasia"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:CCD_RUNX2  RUNX2 — Cleidocranial dysplasia

**STRCHIVE:CCD_RUNX2** — STRchive tandem-repeat disease locus `CCD_RUNX2` (RUNX2, Cleidocranial dysplasia).

## Locus

- STRchive ID: CCD_RUNX2
- Gene: RUNX2
- Gene strand: +
- Disease: Cleidocranial dysplasia
- Disease abbreviation: CCD
- Location type: Coding
- Location in gene: Exon 3
- Inheritance: AD (Autosomal dominant)
- Mechanism: LoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | GCN |
| Pathogenic motif (reference orientation) | GCN |
| Pathogenic motif (gene orientation) | CNG |
| Motif length (bp) | 3 |
| Locus structure | (GCN)* |
| Reference copies | 15 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 4 | 17 |
| Intermediate | - | - |
| Pathogenic | 20 | 27 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr6 | 45422750 | 45422792 |
| hg19 | chr6 | 45390487 | 45390529 |
| T2T-chm13 | chr6 | 45257567 | 45257611 |

## Disease description

A condition that primarily affects the development of the bones and teeth. Characteristic features include underdeveloped or absent collarbones (clavicles); dental abnormalities; and delayed closing of the spaces between the skull bones (fontanels). Other features may include decreased bone density (osteopenia), osteoporosis, hearing loss, bone abnormalities of the hands, and recurrent sinus and ear infections. [@mondo:0007340].

## Mechanism

Polyalanine expansion leading to haploinsufficiency [@pmid:26220009].

## Age of onset

0 (birth) [@genereviews:NBK1513].

## Prevalence

- Details: All conditions 1/1,000,000 births (likely underdiagnosed); Utah population frequency 0.12/10,000. Found in many ethnic groups [@orphanet:1452]. TR expansions causative in 2 individuals [@genereviews:NBK535148].

## Details

Benign range (4-17 repeats) established from gnomAD and primary literature; pathogenic ranges (20-27) reflect two clinical cases to date [@gnomad:RUNX2; @pmid:26220009]. Intermediate alleles (i.e, 18 repeats; 19 not reported) appear to not be associated with disease [@pmid:20560987; @pmid:26220009].

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0007340 |
| OMIM | OMIM:119600 |
| Orphanet | ORPHA:1452 |
| MedGen | 3486 |
| GARD | 6118 |
| MalaCards | CLD001 |
| GeneReviews | NBK1513 |
| gnomAD | RUNX2 |
| STRipy | RUNX2 |
| TR-Atlas | TR65117 |

## References

- genereviews:NBK1513
- pmid:26220009
- gnomad:RUNX2
- pmid:20560987
- orphanet:1452
- genereviews:NBK535148
- pmid:9182765
- mondo:0007340

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
