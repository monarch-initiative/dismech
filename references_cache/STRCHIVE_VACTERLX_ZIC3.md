---
reference_id: "STRCHIVE:VACTERLX_ZIC3"
title: "ZIC3 — X-linked VACTERL syndrome"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:VACTERLX_ZIC3  ZIC3 — X-linked VACTERL syndrome

**STRCHIVE:VACTERLX_ZIC3** — STRchive tandem-repeat disease locus `VACTERLX_ZIC3` (ZIC3, X-linked VACTERL syndrome).

## Locus

- STRchive ID: VACTERLX_ZIC3
- Gene: ZIC3
- Gene strand: +
- Disease: X-linked VACTERL syndrome
- Disease abbreviation: VACTERLX
- Location type: Coding
- Location in gene: Exon 1
- Inheritance: XR (X-linked recessive)
- Mechanism: Unknown

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | GCN |
| Pathogenic motif (reference orientation) | GCN |
| Pathogenic motif (gene orientation) | CNG |
| Motif length (bp) | 3 |
| Locus structure | (GCN)* |
| Reference copies | 9 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 10 | 10 |
| Intermediate | 11 | 11 |
| Pathogenic | 12 | 12 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chrX | 137566826 | 137566856 |
| hg19 | chrX | 136648985 | 136649015 |
| T2T-chm13 | chrX | 135876774 | 135876800 |

## Disease description

VACTERL is an acronym for vertebral anomalies (similar to those of spondylocostal dysplasia), anal atresia, cardiac malformations, tracheoesophageal fistula, renal anomalies (urethral atresia with hydronephrosis), and limb anomalies [@omim:314390].

## Mechanism

Polyalanine expansion with unknown mechanism [@pmid:17581576].

## Age of onset

0

## Prevalence

- Details: Found in one patient [@genereviews:NBK535148] with European ancestry [@pmid:20452998].

## Details

Frequently genotyped as (NGC)*, although locus structure has been reported as (GCC)8(GCT)(GCC) [@pmid:38467784]. 1 patient with VACTERL died at birth with 12 repeats [@pmid:20452998]. 8 patients with X-linked oculo-auriculo-vertebral spectrum (OAVS) had 11 repeats (intermediate allele size); 1 individual in OAVS cohort with 12 repeats; likely phenotypic spectrum [@pmid:32639022].

## Tags

- Locus tags: sparse_evidence
- Disease tags: phenotypic_spectrum

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0010752 |
| OMIM | OMIM:314390 |
| MedGen | 419019 |
| GARD | 15309 |
| MalaCards | VCT005 |
| GeneReviews | NBK535148 |
| gnomAD | ZIC3 |
| STRipy | ZIC3 |
| TR-Atlas | TR173313 |

## References

- pmid:17581576
- pmid:38467784
- pmid:20452998
- pmid:32639022
- genereviews:NBK535148
- omim:314390

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
