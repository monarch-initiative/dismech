...
- Hacohen N
- Diamond B
- Accelerating Medicines Partnership in SLE network
journal: Nat Immunol
year: '2019'
...
insufficiently effective2, 3. Despite the rapid pace of immunologic discovery,
...
decades2, 4. Thus, there is a pressing need to decipher the
immune mechanisms that drive LN.

Current knowledge of the molecular pathways dysregulated in SLE comes mainly from
the unbiased analysis of blood cells5;
...
presence of infiltrating subpopulations of immune cells6, 7, 8 but cannot reveal
...
kidney9 but vary in key aspects
...
It is thus clear that the study of LN can greatly benefit from a resource
...
sequencing (scRNA-seq) using a modified CEL-Seq2 protocol10. Since our focus was on characterizing
...
labeling of cells was highly robust (Supplementary Table 2).

We next clustered the cells of each lineage separately, and identified
...
samples of healthy individuals using scRNA-seq12 (Fig.
...
alternatively activated phenotype17,
...
Finally, since CM2 was the main cluster found in normal kidneys (Supplementary Table 3),
...
microglia and lung resident macrophage populations20, consistent with CM2 representing
...
Dimensionality reduction using either diffusion maps21 (Fig.
3d) or t-Distributed Stochastic Neighbor Embedding22 (tSNE; Fig. 2a) indicated possible transitions between the
...
Table 6) was observed along this trajectory (Fig. 3e,f).

To further investigate this hypothesized within-kidney transition, we
...
identified as CD16+ monocytes.

To determine whether the hypothesized differentiation begins before
...
These analyses are consistent with differentiation of CD16+
...
high levels of the granzyme GZMK24 rather than GZMB and
...
CCR5, consistent with an earlier report25. Cluster CT5 could be further split into
...
rates29 and that such
exhausted T cells are also detected in the kidneys of lupus mice30. In our data, however, all
...
comprehensive, published list of exhaustion markers31 (Supplementary Table 6). We found
...
CXCR5, PDCD1, MAF, and CD200 (Fig. 4c and Supplementary Fig. 4g).

Cluster CT0 could be further split into two subclusters, the first
...
cluster indicated a dysregulation of ISGs in the LN samples (Supplementary Table 5).

While some LN kidney T cells have been previously annotated as
...
Finally, cluster CT6 contained CD4+ T cells demonstrating
...
that are implicated in both aging and autoimmunity33. Based on a panel of genes reported to
be differentially expressed in ABCs34, we computed for each cell in cluster CB0 a score
...
the presence of these cells indeed reflected disease rather than age.

The tSNE plot for the B cells suggested that cluster CB2 may contain
...
FANTOM535,36, containing bulk RNA-seq data from 360
...
cells in the inflamed kidney. However, as it was previously suggested37 that ABCs are preplasma cells,
...
alleles and their susceptibility genes in SLE and LN38, 39. We analyzed the expression of these genes across the 22
...
sensing, B cell activation and differentiation is well established40, 41 is expressed here in pDCs, myeloid cells and B cells.
We found HIP1, suggested to regulate DC activity42, to be expressed in resident
...
modulate synovial hyperplasia43, was expressed here in T- and B-cell subsets, raising the
...
collected from 8 patients, were included in subsequent analyses.

We first assigned each urine cell to the kidney cluster most similar in
...
urine immune cells and corresponding kidney leukocytes.

Our transcriptomic analysis offered a detailed view of the T-cell
populations in LN kidneys. The co-clustering of CD4+ TFH cells and
FoxP3+Helios+ regulatory T cells raises the possibility
that T follicular regulatory cells are also present44, 45.
...
CD8+ T cells in the kidney46 is subset-specific.

B cells are found in more than half of lupus biopsies but not in healthy
samples47. Our finding of
...
responses to tissue damage are being driven in situ48. Of note, activation was correlated with an
ABC signature previously suggested to be driven by BCR/TLR ligands49. Understanding whether these ABCs
...
cells will require a larger data set, combined with analysis of BCR sequences.

Peripheral monocytes can enter injured tissues and differentiate into
inflammatory and reparative/resolving macrophages50. If the cells are chronically exposed to
damage-associated molecular pattern molecules (DAMPs) and endosomal TLR ligands,
resolution may fail, and macrophages with mixed functions may emerge51. Here, the three subpopulations of
...
model of acute inflammation52. Of
...
microenvironmental cues such as cytokines and chemokines.

Our study demonstrates the feasibility of profiling kidney samples using
...
we did not observe a major effect on gene expression due to freezing.

Despite our cohort’s relatively high diversity with respect to
...
scRNA-seq was performed using the CEL-Seq2 method10 with the following modifications. Single
...
carried out as previously described10 and double-stranded c-DNA was purified using 0.8X
...
Frozen needle core biopsies obtained from two additional healthy donor
...
University of North Carolina Kidney Center were used as controls.

For bulk RNA-seq experiments, thawed cells were stained with antibodies
...
sequencing libraries using the Smart-seq2 method53. 38bp paired-end reads were generated on
a Nextseq500 (Illumina).

For droplet-based scRNA-seq, thawed cells were stained with antibodies
...
the Drop-seq pipeline developed by the McCarroll lab54, to perform all steps necessary to
...
10x (all thresholds were set based on empirical distributions).

To minimize the effect of technical factors, we tested different
...
In the analysis of myeloid blood cells, initial comparison to the gene
...
independently for each set of analyzed cells.

Sensitivity analysis was performed in each clustering step, with a
...
genes) remain high in the generated random cells.

Myeloid cells were compared with the scRNA-seq data published in Villani
et al.12,
...
the Benjamini-Hochberg method57. For each gene with a corrected P value smaller than 0.05
