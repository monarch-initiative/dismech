---
reference_id: "STRCHIVE:NME_NAXE"
title: "NAXE — NAXE-related mitochondrial encephalopathy"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:NME_NAXE  NAXE — NAXE-related mitochondrial encephalopathy

**STRCHIVE:NME_NAXE** — STRchive tandem-repeat disease locus `NME_NAXE` (NAXE, NAXE-related mitochondrial encephalopathy).

## Locus

- STRchive ID: NME_NAXE
- Gene: NAXE
- Gene strand: +
- Disease: NAXE-related mitochondrial encephalopathy
- Disease abbreviation: NME
- Location type: 5' UTR
- Inheritance: AR (Autosomal recessive)
- Mechanism: LoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | GGGCC |
| Pathogenic motif (reference orientation) | GGGCC |
| Pathogenic motif (gene orientation) | CCGGG |
| Motif length (bp) | 5 |
| Locus structure | (GGGCC)* |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 2 | 7 |
| Intermediate | - | - |
| Pathogenic | 200 | 200 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr15 | 156591766 | 156591783 |
| hg19 | chr15 | 156561558 | 156561575 |
| T2T-chm13 | chr15 | 155728132 | 155728149 |

## Disease description

Patients with NAXE-related mitochondrial encephalopathy exhibit developmental delay, cognitive regression, altered consciousness, abnormalities in eye movement (including nystagmus), muscle weakness, respiratory failure, seizure, ataxia, and gait disturbance at the age of 1 to 2 years. The disease is characterized by fluctuating symptoms over time, often exacerbated by febrile illness; it is progressive and fatal in the long term [@pmid:39455596].

## Mechanism

Reduced NAXE expression from expansion in promoter; hypermethylation was detected at and downstream of the repeat sequence in the proband as well as the maternal copy of the expanded allele, which was not present in the maternal normal range allele nor in the controls [@pmid:39455596]

## Age of onset

Single repeat expansion case had onset at 13 months, while NAXE-related mitochondrial encephalopathy more generally has predominately infantile onset, extending to 20y or older [@pmid:39455596].

## Prevalence

- Details: Single proband found in Japanese cohort [@pmid:39455596].

## Details

Benign range (2-7) alleles established by 484 control alleles and validated with orthogonal databases, while single proband had expansion of ~200 repeats inherited from mother via uniparental disomy [@pmid:39455596]. While the repeat expansion is newly reported, other variants in the NAXE gene have previously been associated with mitochondrial encephalopathy.

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0020781 |
| OMIM | OMIM:617186 |
| Orphanet | ORPHA:555407 |
| MedGen | 934642 |
| GARD | 17991 |
| MalaCards | ENC069 |
| TR-Atlas | TR7952 |

## References

- pmid:39455596

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
