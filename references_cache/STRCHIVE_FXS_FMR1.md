---
reference_id: "STRCHIVE:FXS_FMR1"
title: "FMR1 — Fragile X syndrome (FXS), fragile X-associated tremor/ataxia syndrome (FXTAS), and fragile X-associated primary ovarian insufficiency FXPOI/POF1"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:FXS_FMR1  FMR1 — Fragile X syndrome (FXS), fragile X-associated tremor/ataxia syndrome (FXTAS), and fragile X-associated primary ovarian insufficiency FXPOI/POF1

**STRCHIVE:FXS_FMR1** — STRchive tandem-repeat disease locus `FXS_FMR1` (FMR1, Fragile X syndrome (FXS), fragile X-associated tremor/ataxia syndrome (FXTAS), and fragile X-associated primary ovarian insufficiency FXPOI/POF1).

## Locus

- STRchive ID: FXS_FMR1
- Gene: FMR1
- Gene strand: +
- Disease: Fragile X syndrome (FXS), fragile X-associated tremor/ataxia syndrome (FXTAS), and fragile X-associated primary ovarian insufficiency FXPOI/POF1
- Disease abbreviation: FXS, FXTAS, POF1
- Location type: 5' UTR
- Inheritance: XD (X-linked dominant)
- Mechanism: LoF/GoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | CGG |
| Pathogenic motif (reference orientation) | CGG |
| Pathogenic motif (gene orientation) | CGG |
| Motif length (bp) | 3 |
| Locus structure | (CGG)* |
| Reference copies | 20.6667 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 5 | 44 |
| Intermediate | 45 | 200 |
| Pathogenic | 201 | 2000 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chrX | 147912037 | 147912111 |
| hg19 | chrX | 146993555 | 146993629 |
| T2T-chm13 | chrX | 146176665 | 146176769 |

## Disease description

A genetic syndrome caused by mutations in the FMR1 gene which is responsible for the expression of the fragile X mental retardation 1 protein. This protein participates in neural development. This syndrome is manifested with mental, emotional, behavioral, physical, and learning disabilities.; Any primary ovarian failure in which the cause of the disease is a mutation in the FMR1 gene.; Fragile X-associated tremor/ataxia syndrome (FXTAS) is a rare neurodegenerative disorder characterized by adult-onset progressive intention tremor and gait ataxia [@mondo:0010383; @mondo:0010706; @mondo:0010382].

## Mechanism

Loss of function via transcriptional silencing in FXS, RNA gain of function in FXTAS/FXPOI [@pmid:16205714; @pmid:36169768].

## Age of onset

Typical: FXS 1 to 'first several years of life', FXTAS 60-65 [@genereviews:NBK1384]; Range: 0 [@url:https://www.uptodate.com/contents/fragile-x-syndrome-clinical-features-and-diagnosis-in-children-and-adolescents] - 78 [@pmid:17427188]; detailed description of typical symptom onset and diagnosis available from Arnold et al [@isbn:978-3-031-66932-3].

## Prevalence

- Prevalence: 14/100000
- Details: Incidence of full mutation in males 19/100,000; prevalence 14/100,000 [@genereviews:NBK1384]. Female prevalence 9/100,000 [@pmid:24700618]. Known carrier frequency is approximately 300-500/100,000 but detected was 11/100,000 [@pmid:29100084]. FXS prevalence 1:7000 males, 1:11,000 females; FX premutation carriers 1:290-855 males, 1:148-300 females [@isbn:978-3-031-66932-3]. Found worldwide [@genereviews:NBK1384]. In Thailand, 1 in 600 women carry a premutation, and 1 in 400 carry a 'gray zone' allele [@pmid:39320553].

## Details

Intermediate or 'gray zone' occur at 45-54 alleles and may be unstable enough to expand into the premutation range, as well as associate with parkinsonism [@pmid:32463542; @genereviews:NBK1384]. FXTAS/POI occurs at 55-200 repeats, FXS >200, late onset; AGG and CTG interruptions documented [@genereviews:NBK1384; @pmid:29868108].

## Tags

- Locus tags: somatic_instability, anticipation, maternal_expansion, length_affects_onset, length_affects_penetrance, length_affects_phenotype, length_affects_severity, motif_affects_instability
- Disease tags: phenotypic_spectrum, ataxia

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0010383 |
| MONDO | MONDO:0010706 |
| MONDO | MONDO:0010382 |
| OMIM | OMIM:300624 |
| OMIM | OMIM:300623 |
| Orphanet | ORPHA:908 |
| Orphanet | ORPHA:642691 |
| Orphanet | ORPHA:93256 |
| MedGen | 8912 |
| MedGen | 1644269 |
| MedGen | 333403 |
| GARD | 6464 |
| GARD | 16806 |
| MalaCards | FRG001 |
| MalaCards | FRG008 |
| MalaCards | PRM405 |
| GeneReviews | NBK1384 |
| gnomAD | FMR1 |
| STRipy | FMR1 |
| TR-Atlas | TR173944 |

## References

- genereviews:NBK1384
- url:https://www.uptodate.com/contents/fragile-x-syndrome-clinical-features-and-diagnosis-in-children-and-adolescents
- pmid:17427188
- isbn:978-3-031-66932-3
- pmid:16205714
- pmid:36169768
- pmid:32463542
- pmid:29868108
- pmid:24700618
- pmid:29100084
- pmid:39320553
- pmid:1605194
- pmid:1710175
- mondo:0010383
- mondo:0010706
- mondo:0010382

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
