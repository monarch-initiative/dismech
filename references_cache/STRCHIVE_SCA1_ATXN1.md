---
reference_id: "STRCHIVE:SCA1_ATXN1"
title: "ATXN1 — Spinocerebellar Ataxia Type 1"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:SCA1_ATXN1  ATXN1 — Spinocerebellar Ataxia Type 1

**STRCHIVE:SCA1_ATXN1** — STRchive tandem-repeat disease locus `SCA1_ATXN1` (ATXN1, Spinocerebellar Ataxia Type 1).

## Locus

- STRchive ID: SCA1_ATXN1
- Gene: ATXN1
- Gene strand: -
- Disease: Spinocerebellar Ataxia Type 1
- Disease abbreviation: SCA1
- Location type: Coding
- Location in gene: Exon 8
- Inheritance: AD (Autosomal dominant)
- Mechanism: GoF/LoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | CTG |
| Pathogenic motif (reference orientation) | CTG |
| Pathogenic motif (gene orientation) | AGC |
| Motif length (bp) | 3 |
| Locus structure | (CTG)* |
| Reference copies | 30.3 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 6 | 35 |
| Intermediate | 36 | 38 |
| Pathogenic | 39 | 91 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr6 | 16327634 | 16327724 |
| hg19 | chr6 | 16327865 | 16327955 |
| T2T-chm13 | chr6 | 16200189 | 16200282 |

## Disease description

Spinocerebellar ataxia type 1 (SCA1) is a subtype of type I autosomal dominant cerebellar ataxia (ADCA type I) characterized by dysarthria, writing difficulties, limb ataxia, and commonly nystagmus and saccadic abnormalities [@mondo:0008119].

## Mechanism

Polyglutamine expansion leading to toxic gain of function with eventual misregulation-based loss of function/dominant negative [@genereviews:NBK1184; @pmid:35573049].

## Age of onset

Typical: 20-39 [@url:https://www.uptodate.com/contents/autosomal-dominant-spinocerebellar-ataxias]; Range: 6 [@pmid:3165612] - 63 [@pmid:8825276].

## Prevalence

- Prevalence: 1.5/100000
- Details: 1-2/100,000. Cases have been reported worldwide, although prevalences varies by ancestry/ethnicity [@genereviews:NBK1184].

## Details

Penetrance is dependent on sequence purity in addition to expansion length: pure repeats are pathogenic at 39 repeats [@pmid:37906407], while CAT interruptions [@pmid:35245110] can lead to reduced penetrance at comparable lengths [@genereviews:NBK1184]. Regardless, intermediate alleles are considered premutations which may lead to disease upon transmission [@genereviews:NBK1184].

## Tags

- Locus tags: somatic_instability, anticipation, paternal_expansion, length_affects_onset, length_affects_penetrance, length_affects_severity, motif_affects_instability, motif_affects_onset, motif_affects_penetrance, motif_affects_severity
- Disease tags: spinocerebellar_ataxia

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0008119 |
| OMIM | OMIM:164400 |
| Orphanet | ORPHA:98755 |
| MedGen | 155703 |
| GARD | 4071 |
| MalaCards | SPN294 |
| GeneReviews | NBK1184 |
| gnomAD | ATXN1 |
| STRipy | ATXN1 |
| TR-Atlas | TR63118 |

## References

- url:https://www.uptodate.com/contents/autosomal-dominant-spinocerebellar-ataxias
- pmid:3165612
- pmid:8825276
- genereviews:NBK1184
- pmid:35573049
- pmid:37906407
- pmid:35245110
- pmid:8358429
- mondo:0008119

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
