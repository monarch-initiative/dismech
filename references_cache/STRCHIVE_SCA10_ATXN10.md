---
reference_id: "STRCHIVE:SCA10_ATXN10"
title: "ATXN10 — Spinocerebellar Ataxia Type 10"
database: "STRchive"
content_type: "structured_record"
---

# STRCHIVE:SCA10_ATXN10  ATXN10 — Spinocerebellar Ataxia Type 10

**STRCHIVE:SCA10_ATXN10** — STRchive tandem-repeat disease locus `SCA10_ATXN10` (ATXN10, Spinocerebellar Ataxia Type 10).

## Locus

- STRchive ID: SCA10_ATXN10
- Gene: ATXN10
- Gene strand: +
- Disease: Spinocerebellar Ataxia Type 10
- Disease abbreviation: SCA10
- Location type: Intronic
- Location in gene: Intron 9
- Inheritance: AD (Autosomal dominant)
- Mechanism: GoF

## Repeat

| Field | Value |
|---|---|
| Reference motif (reference orientation) | ATTCT |
| Pathogenic motif (reference orientation) | ATTCT |
| Pathogenic motif (gene orientation) | ATTCT |
| Motif length (bp) | 5 |
| Locus structure | (ATTCT)* |
| Reference copies | 14 |

## Repeat-count thresholds

Allele repeat counts (number of motif copies) by pathogenicity category. Bounds are inclusive as reported by STRchive; a blank bound is unbounded or not documented.

| Category | Min copies | Max copies |
|---|---|---|
| Benign | 10 | 32 |
| Intermediate | 33 | 850 |
| Pathogenic | 800 | 4500 |

## Genomic coordinates

| Build | Chrom | Start | Stop |
|---|---|---|---|
| hg38 | chr22 | 45795355 | 45795424 |
| hg19 | chr22 | 46191235 | 46191304 |
| T2T-chm13 | chr22 | 46280060 | 46280129 |

## Disease description

Spinocerebellar ataxia type 10 (SCA10) is a subtype of type I autosomal dominant cerebellar ataxia (ADCA type I). It is characterized by slowly progressive cerebellar syndrome and epilepsy, sometimes mild pyramidal signs, peripheral neuropathy and neuropsychological disturbances [@mondo:0011330].

## Mechanism

Transdominant mechanism theorized [@pmid:38467784].

## Age of onset

Typical: 12-48; Range: 11-83 [@genereviews:NBK1175].

## Prevalence

- Details: Unknown prevalence, >300 individuals. Cases have been identified in Mexico, Brazil, China, and Japan [@genereviews:NBK1175].

## Details

Unaffected individuals are usually (82%) compound heterozygotes in the benign range [@genereviews:NBK1175]. Intermediate alleles show reduced penetrance, and exact distinction between intermediate and the lower end of the pathogenic range is unclear [@genereviews:NBK1175]. Expansions are frequently interrupted by ATCCT, ATCCC, ATTCC, ATTTCT, ATATTCT, ATTCTTCT, or ATTCTTCT; interruptions of ATTGT, TTTCT, ATTTTCT, ATTCTCT have been noted [@pmid:36199580] as has the interruption ATGCT [@pmid:19234597]. The ATCCT interruption motif is associated with a higher prevalence of epileptic seizures [@pmid:24318420].

## Tags

- Locus tags: somatic_instability, anticipation, paternal_expansion, length_affects_onset, length_affects_penetrance, length_affects_phenotype, motif_affects_instability, motif_affects_penetrance, motif_affects_phenotype
- Disease tags: spinocerebellar_ataxia

## Cross-references

| Database | ID |
|---|---|
| MONDO | MONDO:0011330 |
| OMIM | OMIM:603516 |
| Orphanet | ORPHA:98761 |
| MedGen | 369786 |
| GARD | 10474 |
| MalaCards | SPN314 |
| GeneReviews | NBK1175 |
| gnomAD | ATXN10 |
| STRipy | ATXN10 |
| TR-Atlas | TR165509 |

## References

- genereviews:NBK1175
- pmid:38467784
- pmid:36199580
- pmid:19234597
- pmid:24318420
- pmid:11017075
- pmid:8358429
- mondo:0011330

## Source

STRchive (https://strchive.org), version **2.1.0** (snapshot 2026-07-23). A centralized catalog of tandem-repeat disease loci. Content is for research use and does not constitute medical guidance.

License: CC BY 4.0.

[STRchive](https://strchive.org) · [source repository](https://github.com/hdashnow/STRchive)
