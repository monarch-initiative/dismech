...
- `particulate_air_pollution_injury` — Conserved injury pathway of ambient air pollution, the pollutant-agnostic complement of the tobacco-smoke and occupational-dust routes to the same hub: deposition of fine particulate matter and oxidant gases (PM2.5, PM10, black carbon, ozone, NO2, SO2, wildfire smoke) on airway epithelium and alveolar macrophages → airway epithelial and macrophage oxidative stress → NF-kB activation and NLRP3 inflammasome assembly with caspase-1-dependent IL-1beta/IL-18 maturation → neutrophilic airway inflammation and cytokine release → two parallel consequence branches, airway epithelial barrier disruption/remodeling and systemic inflammation with endothelial dysfunction (the seam handing off to `atherogenesis`). Deliberately does NOT re-derive the protease-antiprotease arm (`emphysema_protease_antiprotease_imbalance`), the mesenchymal arm (`fibrotic_response`), the type-2 sensitization arm of barrier loss (`epithelial_barrier_dysfunction`), or the plaque-formation arm (`atherogenesis`), each of which a conformer may declare in parallel; the PAH mutagenesis/lung-carcinogenesis route and carbon monoxide (a co-emitted combustion product acting by a separate hypoxic mechanism — see `Carbon_Monoxide_Poisoning`) are explicitly out of scope and must not be curated as conforming. Curators should bind the disease-level exposure to the **specific** ECTO term rather than the generic parent `ECTO:8000036` exposure to air pollution — `ECTO:7000117` fine respirable suspended particulate matter (PM2.5), `ECTO:7000116` (PM10), `ECTO:7000068` particulate matter, `ECTO:9000052` ozone, `ECTO:9000252` nitrogen dioxide, `ECTO:9000196` sulfur dioxide, `ECTO:9002171` carbon black, `ECTO:7000059` smoke — and link it to the conforming node with `influences_mechanisms`. Evidence-discipline caveat conformers must preserve: the ambient-concentration epidemiology is far better evidenced in humans than the molecular hub, which rests chiefly on in vitro airway-cell and rodent exposure work, so association and mechanism evidence stay separate items with distinct `evidence_source` values. Not an Xogenesis module (loss of an intact barrier and of normal endothelial function, not formation of a pathological structure). Worked conformer: Chronic_Obstructive_Pulmonary_Disease (amplifier + central_effector nodes, plus a PM2.5-specific `ECTO:7000117` exposure linked to its NLRP3 node). Key conformance target: `particulate_air_pollution_injury#NF-kB Activation and NLRP3 Inflammasome Assembly`
