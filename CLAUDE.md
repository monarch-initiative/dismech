...
> **Status:** Proposed decision rule, pending maintainer ratification ([#3881](https://github.com/monarch-initiative/dismech/issues/3881)). It codifies current practice: ~98.6% of current entries (3,008 of 3,052) carry a top-level MONDO `disease_term`. The 44 exceptions are the worklist of [#3691](https://github.com/monarch-initiative/dismech/issues/3691) and are individually untriaged — each is *expected* to resolve to rule 1, 2, or 3 below, and none is asserted to be a standing exception. Some are plainly rule-1 entries nobody has anchored yet (`Adenovirus_Respiratory_Infection`, `Human_Metapneumovirus_Infection`, `Seasonal_Coronavirus_Infection`); others are exactly the boundary calls #3881 exists to settle (`Volumetric_Muscle_Loss` models itself as the fibrotic response specialized to skeletal muscle; `Chemotherapy_Induced_Nausea_and_Vomiting` is `category: Iatrogenic`; `Spaceflight_Associated_Neuro-Ocular_Syndrome` lists a `Spaceflight-Induced Condition` parent). Do not read the figure as a claim that those calls have been made.
...
3. **No — it is a recurring pathophysiological *process / state*** (e.g.
   ...
   **Caveat:** module representation of processes and states is under active
   refinement and may not yet cleanly hold every entity this branch routes to it
   — the destination is the right one, but expect to find the modelling rough.
   If your process or state does not fit, say so on
   [#3881](https://github.com/monarch-initiative/dismech/issues/3881) rather
   than forcing it into a module or falling back to an anchorless disease entry.
...
**Not yet propagated to the agent skills.** This rule currently lives only
here. `.claude/skills/initiate-new-disorder-creation/SKILL.md` assumes a MONDO
anchor in its `disease_term` scaffold and its review checklist but encodes
neither the four-branch decision nor the rule-2 NTR workflow, so an agent
driving that skill will not apply this. Propagating it is tracked as part of
ratification under
[#3881](https://github.com/monarch-initiative/dismech/issues/3881); docs first,
skills second, and this note comes out when the skill carries the rule.
