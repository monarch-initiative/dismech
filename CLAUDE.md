...
- `phototransduction_cascade_dysfunction` — Conserved photoreceptor-intrinsic failure of the cGMP phototransduction cascade shared by the non-degenerative (or slowly degenerative) photoreceptor signalling diseases: phototransduction cascade component defect (activation arm — visual pigment, transducin GNAT1/GNB3, PDE6, CNG channel CNGA3/CNGB3, guanylate cyclase/GCAP; or shutoff arm — GRK1, SAG, RGS9/RGS9BP) → loss of outer-segment cGMP and Ca2+ set-point control → **failure of photoreceptor light-response generation or timely recovery** → collapse of the photoreceptor-derived ERG a-wave and impaired dark adaptation → congenital, characteristically non-progressive photoreceptor dysfunction (night blindness for rod lesions; photophobia, nystagmus, reduced acuity and colour loss for cone lesions), with a conditional branch handing off to `photoreceptor_degeneration` when cGMP/Ca2+ sit at chronically toxic levels. Carries the AAV cascade-gene supplementation drug-target pattern (`RESTORES` on the rate-limiting node). **Scope guardrail:** the Schubert-Bornschein CSNB genes (NYX, GRM6, TRPM1, GPR179, LRIT3, CACNA1F) are a photoreceptor-to-ON-bipolar *transmission* defect with the reciprocal ERG signature (preserved a-wave, reduced b-wave) and must NOT be wired here — a CSNB entry conforms only for its Riggs arm. Carries an open `KNOWLEDGE_GAP` on why the same gene (GNAT1, CNGA3/CNGB3) can give either a stationary or a degenerative phenotype. Worked conformers: Achromatopsia, Blue_Cone_Monochromacy, Congenital_Stationary_Night_Blindness, PDE6A-Related_Retinopathy, GUCY2D-Related_Retinopathy, RHO-Related_Retinopathy. Key conformance / treatment target: `phototransduction_cascade_dysfunction#Failure of Photoreceptor Light-Response Generation or Timely Recovery`
- `retinoid_visual_cycle_disruption` — Conserved defect of the retinoid (visual) cycle that regenerates 11-cis-retinal between the photoreceptor outer segment and the RPE: visual cycle enzyme/transporter defect (RPE65, LRAT, RDH5, RLBP1, RDH12, ABCA4, STRA6/RBP4) → impaired retinoid flux between photoreceptor and RPE → **chromophore insufficiency and retinoid intermediate accumulation** → RPE lipofuscin/bisretinoid loading and RPE dysfunction, and in parallel delayed dark adaptation and night blindness → progressive photoreceptor and RPE degeneration (seam to `photoreceptor_degeneration`). **The two arms take opposite therapeutic directions and the module carries both explicitly:** a chromophore-supply lesion is treated by *increasing* flux (RPE65 gene replacement, voretigene neparvovec, `RESTORES`), a clearance lesion by *decreasing* it (visual-cycle modulators, deuterated vitamin A, `INHIBITS`) — wiring the wrong one inverts the mechanism. Distinct from `phototransduction_cascade_dysfunction`, which consumes the chromophore this module supplies. Worked conformers: Stargardt_Disease and RPE65-Related_Retinopathy (opposite arms), RDH5-Related_Retinopathy. Key conformance target: `retinoid_visual_cycle_disruption#Chromophore Insufficiency and Retinoid Intermediate Accumulation`
- `anterior_segment_dysgenesis` — Conserved neural-crest developmental failure behind the anterior segment dysgenesis spectrum (aniridia, Axenfeld-Rieger, Peters anomaly, primary congenital glaucoma, primary aphakia): anterior segment developmental regulator defect (PAX6, PITX2, FOXC1, FOXE3, PITX3, LMX1B, TFAP2B; CYP1B1, LTBP2, PXDN, B3GLCT), usually dose-sensitive → disrupted periocular mesenchyme migration and differentiation → **failed differentiation and separation of anterior segment tissues** → structural anterior segment malformation as a recurring multi-tissue *bundle* (iris hypoplasia/corectopia, posterior embryotoxon, peripheral anterior synechiae, corneal opacity, lens anomaly) → maldeveloped aqueous outflow pathway and developmental glaucoma. The anterior-segment analogue of the serial-homology modules: one progenitor population builds cornea, iris, angle, trabecular meshwork and Schlemm's canal, so the angle is involved whether or not it is the obvious lesion. Deliberately NOT `ocular_morphogenesis_failure` (the earlier optic-vesicle/cup/fissure events ending in the A-M-C spectrum), though a syndromic entry may conform to both; hands off to `glaucoma_optic_neuropathy#Trabecular Meshwork Outflow Dysfunction` for what raised pressure then does. Carries an open `KNOWLEDGE_GAP` on whether glaucoma risk is predictable from the visible malformation. Worked conformers: Aniridia, Axenfeld-Rieger_syndrome, Peters_Plus_Syndrome, Congenital_Glaucoma. Key conformance target: `anterior_segment_dysgenesis#Failed Differentiation and Separation of Anterior Segment Tissues`

- `norrin_fzd4_retinal_vascular_development` — Conserved inherited-retinal-vascular-disease module: Norrin ligand, receptor or co-receptor defect (NDP, FZD4, LRP5, TSPAN12; also ZNF408, KIF11 in FEVR pedigrees) → **insufficient Norrin-FZD4-beta-catenin signalling in retinal endothelium** → two separable arms, incomplete retinal vascularization with a peripheral avascular retina (the angiogenic arm) and inner blood-retina barrier failure (the barrier arm, attenuated by partial Wnt7a/Wnt7b redundancy) → ischaemia-driven neovascularization, exudation and tractional detachment. The phenotypic continuum from asymptomatic peripheral avascularity through exudative vitreoretinopathy to congenital retinal dysplasia is modelled as a **dose series on one pathway**, not a set of distinct mechanisms. Carries the investigational pathway-agonism drug-target pattern (`ACTIVATES` on the rate-limiting node), distinct from anti-VEGF therapy which targets the neovascular consequence. **Scope guardrail:** retinopathy of prematurity and proliferative diabetic retinopathy converge on the same ischaemic neovascular endpoint by different routes and must NOT be wired here — a conformer needs a lesion in the Norrin-FZD4 system itself. Note LRP5's non-retinal pleiotropy (osteopenia/osteoporosis) belongs on the disorder entry, not the module. Worked conformers: Familial_Exudative_Vitreoretinopathy (four nodes) and NDP_Related_Vitreoretinopathy. Key conformance / treatment target: `norrin_fzd4_retinal_vascular_development#Insufficient Norrin-FZD4-beta-catenin Signalling in Retinal Endothelium`
