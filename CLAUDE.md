...
- `cns_myelin_failure` — Conserved CNS oligodendrocyte/myelin failure, the mechanism module behind the **myelin-disorder** arm of the leukodystrophies: a heritable lesion in a myelin structural protein (PLP1, GJC2), in oligodendrocyte-essential transcription/biogenesis machinery (POLR3A/POLR3B, PYCR2, TUBB4A), or in a lipid-degradation pathway whose toxic substrate is enriched in myelinating cells (ARSA/sulfatide, GALC/psychosine, ABCD1/VLCFA, ASPA/NAA) → oligodendrocyte differentiation arrest and death, in parallel with microglial injury and reactive gliosis (microglial loss *precedes* myelin breakdown in X-ALD and MLD, the rationale for microglia-replenishing HSCT) → deficient or unstable CNS myelin sheath, in one of three forms (hypomyelination / demyelination / myelin vacuolization) → loss of oligodendroglial metabolic and trophic support to the axon (MCT1 lactate shuttle) → axonal degeneration and progressive white-matter dysfunction. **Scope guardrails, and they matter:** this is only van der Knaap & Bugiani's *myelin-disorder* category (PMID:28638987) — astrocytopathies (Alexander disease/GFAP, vanishing white matter/eIF2B), leuko-axonopathies, microgliopathies and leuko-vasculopathies are deliberately NOT conformers at the trigger or oligodendrocyte nodes, though they may conform at the sheath node when myelin loss is separately evidenced; white-matter MRI signal change alone never makes a conformer; acquired demyelination (MS, MOGAD, ADEM, osmotic) is out of scope at the trigger; and a Schwann-cell/PNS demyelinating arm belongs to `peripheral_axonal_degeneration`, not here. Complementary to `lysosomal_substrate_accumulation` and `peroxisomal_metabolic_failure`, which explain how the toxic substrate arises — a conformer declares both, at different nodes. Worked conformers: Pelizaeus_Merzbacher_Disease (5 nodes, the flagship), Krabbe_Disease, Metachromatic_Leukodystrophy, adrenoleukodystrophy, POLR-Related_Leukodystrophy, Hypomyelinating_Leukodystrophy_7/10, Canavan_Disease, Adult-Onset_Autosomal_Dominant_Demyelinating_Leukodystrophy. Key conformance target: `cns_myelin_failure#Deficient or Unstable CNS Myelin Sheath`
- `corticospinal_tract_axonopathy` — Conserved hereditary-spastic-paraplegia final common pathway and the **CNS counterpart of `peripheral_axonal_degeneration`**: a lesion in one of the housekeeping systems a very long axon depends on (microtubule severing/transport SPG4-SPG10-SPG30; ER morphology SPG3A-SPG31-SPG12; mitochondrial quality control SPG7-SPG13; endolysosomal/vesicle traffic AP-4 and AP-5 disorders; protein folding SPG6-SPG8-SPG17; lipid metabolism SPG28-SPG35-SPG54-SPG56) → impaired axonal transport and organelle distribution, with focal axonal swellings that worsen transport distal to them → distal length-dependent degeneration of long CNS axons (corticospinal maximal in thoracic cord, fasciculus gracilis maximal cervico-medullary — opposite ends of the cord, same reason: distance from the soma) → loss of supraspinal inhibitory control of the stretch reflex → progressive lower-limb spasticity and weakness. The two modules model the same cell-biological failure in different compartments and give opposite bedside signs (spasticity/hyperreflexia vs. distal weakness/areflexia); a disorder with both arms conforms to both, at different nodes — never collapse them. **Excluded on purpose:** the myelin-formation SPGs (SPG2/PLP1, SPG42/GJC2 → `cns_myelin_failure`) and the neurodevelopmental SPGs (SPG1/L1CAM, SPG22/MCT8), since a tract that never forms is not a tract that fails to be maintained. Worked conformers: Hereditary_Spastic_Paraplegia (trigger + effector + consequence), Complex_Hereditary_Spastic_Paraplegia, Hereditary_Spastic_Paraplegia_48, CPT1C-Related_Hereditary_Spastic_Paraplegia, UCHL1-Related_Neurodegeneration_with_Optic_Atrophy_and_Spastic_Paraplegia, ALDH18A1_De_Barsy_Spectrum, Behr_Syndrome, Friedreich_Ataxia. Key conformance target: `corticospinal_tract_axonopathy#Distal Length-Dependent Degeneration of Long CNS Axons`
- `peroxisomal_metabolic_failure` — Conserved peroxisomal-disorder pattern spanning both the biogenesis disorders and the single-enzyme/transporter defects: a PEX-gene lesion impairing membrane assembly, matrix-protein import or organelle division, or loss of a single peroxisomal transporter/enzyme → failure of peroxisomal fatty-acid beta-/alpha-oxidation and of ether-lipid (plasmalogen) synthesis → the characteristic **two-sided** lesion, toxic fatty-acid accumulation *with* ether-lipid deficiency → neural membrane and redox dysfunction (plasmalogens are the most abundant ether phospholipid of myelin and are required cell-intrinsically for an oligodendrocyte to ensheath an axon; VLCFAs drive ROS and lipid peroxidation) → multisystem peroxisomal disease. **The arms are not interchangeable and a conformer must attach at the sub-branch its own biochemistry supports:** the generalized biogenesis arm (ZSD-PEX genes) fails everything at once, whereas PEX7/RCDP1 fails only PTS2 import (VLCFAs are *normal*), GNPAT/AGPS fail plasmalogen synthesis alone with no fatty-acid accumulation, ABCD1 fails only VLCFA beta-oxidation, and PHYH fails only phytanic-acid alpha-oxidation — curating a normal-range analyte as accumulated is the likeliest error here. Congenital neuronal migration defects are a biogenesis-arm developmental feature, not a downstream consequence of the postnatal metabolic chain. Structurally parallel to but not interchangeable with `lysosomal_substrate_accumulation` (different organelle, different substrates); hands off to `cns_myelin_failure` for the leukodystrophy arm. Worked conformers: Zellweger_Spectrum_Disorders, Rhizomelic_Chondrodysplasia_Punctata_Type_1, Rhizomelic_Chondrodysplasia_Punctata_Plasmalogen_Synthesis_Defect, Adult_Refsum_Disease, adrenoleukodystrophy. Key conformance target: `peroxisomal_metabolic_failure#Toxic Fatty-Acid Accumulation with Ether-Lipid Deficiency`
