...
- `nlrp3_inflammasome_activation` — Conserved two-signal NLRP3 inflammasome pathway, the innate-immune sensing device that converts heterogeneous danger signals into one stereotyped inflammatory output: NF-kappaB-dependent priming and NLRP3 transcriptional upregulation (signal 1, licensing but not activating) → activating danger signal (signal 2 — potassium efflux, lysosomal rupture after phagocytosis of particulates, or mitochondrial ROS/mtDNA release; a union of non-exclusive routes) → NLRP3-ASC assembly and proximity-induced caspase-1 activation → IL-1 beta/IL-18 maturation and gasdermin D-mediated pyroptosis → sterile inflammatory tissue injury. Carries **two** complementary drug-target patterns: output blockade (canakinumab `INHIBITS` the cytokine node; CANTOS is the human proof, with more fatal infections as the honest caveat) and sensor blockade (MCC950 `INHIBITS` the assembly node — curated as a mechanistic design pattern only, since it is investigational and its RA trial was suspended for hepatic toxicity, so conformers must not curate it as therapy). Two conformance rules that are not obvious from the chain: a conformer may split signal 2 across several atomic nodes that each conform to the single `#Activating Danger Signal` node, and a **gain-of-function NLRP3 variant enters at the central effector and must NOT be wired to `#Activating Danger Signal`** — the point of the lesion is that no second signal is needed. Scoped to the canonical NLRP3 sensor: NLRP1/NLRC4/AIM2/pyrin entries curate their own node. Deliberately does not re-derive `mitochondrial_dysfunction` or `disabled_macroautophagy` (which own the clearance arm of the mitochondrial route), and complements `cellular_senescence`/`inflammaging`, which describe sterile inflammation without owning an inflammasome node. Carries a `KNOWLEDGE_GAP` recording that the three activating routes conflict as unifying models (PMID:23809161 found mitochondrial perturbation dispensable; PMID:21124315 found the mitophagy block sufficient), so no conformer may assert one as *the* mechanism of NLRP3 activation; and an `OPEN_QUESTION` on whether `gout_urate_crystal_inflammation` should conform here or keep its own crystal-specific node — that module is deliberately left untouched. Worked conformers: Silicosis (particulate route; conforms at the activating-signal node twice and at the assembly node) and Familial_Cold_Autoinflammatory_Syndrome (constitutive-variant route; assembly and cytokine-output nodes). Key conformance target: `nlrp3_inflammasome_activation#NLRP3 Inflammasome Assembly and Caspase-1 Activation`
