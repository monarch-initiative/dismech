...
- `vegf_inhibitor_hypertension` — Conserved on-target vascular-toxicity pattern for VEGF pathway inhibitors (bevacizumab, aflibercept, ramucirumab; the VSP tyrosine kinase inhibitors sunitinib, sorafenib, axitinib, pazopanib, lenvatinib, cabozantinib): VEGF signalling pathway inhibition in vascular endothelium → three parallel arms — endothelial nitric oxide deficiency with oxidative stress, endothelin-1 system activation, and microvascular rarefaction — → vasoconstriction and increased peripheral vascular resistance → treatment-emergent (dose-limiting, withdrawal-reversible) hypertension. Carries the antihypertensive drug-target pattern (ACE inhibitor/ARB and calcium channel blocker treatments use `target_mechanisms` with `INHIBITS` on the convergence node rather than restoring the VEGF trigger, which would antagonize the antitumour effect). Two curated framing guardrails: the integrated mechanism is explicitly unsettled (`KNOWLEDGE_GAP`; the AHA statement calls the molecular mechanisms unclear), and the reported association of treatment-emergent hypertension with antitumour efficacy is an `OPEN_QUESTION`, not an established prognostic claim. Complements `tumor_angiogenesis` (the therapeutic rationale this toxicity is the price of) and `drug_induced_nephrotoxicity` (the separate renal arm of VEGF inhibition). Worked conformer: `Secondary_Hypertension` (VEGF-inhibitor drug-cause arm). Key conformance / treatment target: `vegf_inhibitor_hypertension#Vasoconstriction and Increased Peripheral Vascular Resistance`
