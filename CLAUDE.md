...
- `enterocyte_polarity_trafficking_failure` — Conserved final-common pathway of the **enterocyte-structural** congenital diarrheas, the third and most severe of the three epithelial CODE classes (PMID:29654747) and the sibling of `diet_induced_osmotic_diarrhea` and `electrolyte_transport_related_diarrhea`: loss of the apical-delivery machinery (MYO5B motor, STX3 SNARE) **or** of junctional adhesion (EpCAM, directly or via SPINT2-unleashed matriptase) — curated as **two separate trigger nodes**, since a blended trigger let a conformer implicitly assert the other arm's lesion; arm selection is the curator's first decision and a disorder must enter through exactly one → loss of a functionally polarized absorptive epithelial surface (the rate-limiting node) → abnormal villus architecture and, in parallel, feeding-independent intractable diarrhea → intestinal failure with parenteral-nutrition dependence. **The discriminator against both sibling modules is the villus-to-crypt ratio** — abnormal here, normal in both of the others — and a disorder must not conform to this and either sibling for the same node. Deliberately admits two molecularly distinct trigger arms that converge but are NOT equivalent (intracellular microvillus inclusions vs focal epithelial tufts), so conformers substitute their own arm's lesion. Excludes immune-mediated enteropathy (IPEX), barrier injury (`intestinal_barrier_dysfunction`), and post-uptake intracellular defects (DGAT1 deficiency, where the apical surface is intact). Treatment patterns: parenteral nutrition `BYPASSES` the diarrhea node, and small-bowel transplantation `RESTORES` the central effector — the only curative intervention across the three CODE modules, and the therapeutic signature of this class. Worked conformers: Microvillus_Inclusion_Disease (4 nodes, trafficking arm), Congenital_Tufting_Enteropathy (5 nodes, adhesion arm). Key conformance target: `enterocyte_polarity_trafficking_failure#Loss of a Functionally Polarized Absorptive Epithelial Surface`
- `electrolyte_transport_related_diarrhea` — Conserved final-common pathway of the **electrolyte-transport-related** congenital diarrheas, the deliberate sibling of `diet_induced_osmotic_diarrhea` in the CODE nosology (PMID:29654747): loss of one apical intestinal electrolyte-transport step — the Cl-/HCO3- exchanger DRA (SLC26A3), or the Na+/H+ exchanger NHE3 (SLC9A3, lost either directly or by constitutive inhibition from an activating GUCY2C variant upstream, since NHE3 is a downstream target of GC-C) — → luminal electrolyte retention and failure of net ion absorption (the rate-limiting node) → obligated water loss and feeding-independent watery diarrhea with a LOW stool osmotic gap, and, in parallel, ion-specific systemic depletion whose acid-base direction follows the lost transporter's coupling partner: hypochloremic metabolic **alkalosis** for DRA (bicarbonate retained), hyponatremic metabolic **acidosis** for NHE3 (protons retained). The two conformers are acid-base mirror images of one shared mechanism — the module's main predictive payoff and the fastest bedside discriminator. Because the failed step handles an ION the body secretes and recycles rather than an ingested nutrient, the diarrhea persists through bowel rest and no elimination diet is therapeutic; a disorder must not conform to both this and `diet_induced_osmotic_diarrhea` for the same node. The older "secretory" label is deliberately avoided: the primary lesion is failure of ion ABSORPTION. Two treatment patterns on different nodes: oral electrolyte substitution `INHIBITS` the systemic node (life-saving, no effect on stool volume), and sodium butyrate `ACTIVATES` the trigger (the only curated intervention that reduces stool output — chloride-conformer-specific and genotype-dependent, NOT inherited by the sodium conformer). Worked conformers: Congenital_Chloride_Diarrhea (4 nodes), Congenital_Sodium_Diarrhea (5 nodes). Key conformance target: `electrolyte_transport_related_diarrhea#Luminal Electrolyte Retention and Failure of Net Ion Absorption`
- `diet_induced_osmotic_diarrhea` — Conserved final-common pathway of the "diet-induced" (classically osmotic) congenital diarrheas, deliberately scoped against the other CODE classes (PMID:29654747): loss of one substrate-specific apical step at the enterocyte brush border — a disaccharidase (LCT, SI, TREH) or the apical monosaccharide carrier SGLT1/SLC5A1 — → unabsorbed dietary solute retained in the intestinal lumen (the rate-limiting node) → osmotic water influx and luminal distension, with a parallel colonic microbial fermentation branch whose short-chain-fatty-acid salvage arm sets the symptomatic dose threshold → substrate-dependent watery diarrhea that remits on dietary withdrawal and recurs on rechallenge. Villus architecture stays normal (normal villus-to-crypt ratio), which is the boundary against `intestinal_barrier_dysfunction` (malabsorption there is *downstream* of mucosal injury) and against the structural/trafficking and immune-mediated enteropathies; electrolyte-transport-related diarrhea (SLC26A3, SLC9A3, GUCY2C) is out of scope. Carries two complementary treatment patterns on different nodes: dietary substrate elimination `INHIBITS` the luminal-retention node, and oral enzyme replacement (sacrosidase) `BYPASSES` the trigger — the latter is hydrolase-class-specific and is NOT inherited by transporter-class conformers. Worked conformers: Congenital_Lactase_Deficiency, Congenital_Sucrase-Isomaltase_Deficiency, Trehalase_Deficiency (all four nodes), Glucose-Galactose_Malabsorption (the transporter-class arm). Key conformance target: `diet_induced_osmotic_diarrhea#Unabsorbed Dietary Solute Retention in the Intestinal Lumen`
