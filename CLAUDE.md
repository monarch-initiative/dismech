...
- `epigenetic_machinery_neurodevelopmental_dysregulation` — Conserved mechanism of the **Mendelian disorders of the epigenetic machinery** (MDEMs / chromatinopathies), collectively a common cause of intellectual disability: haploinsufficiency of a dosage-sensitive chromatin **writer, eraser, reader, or remodeler** → imbalance between permissive and repressive chromatin states at that component's target loci → dysregulated neurodevelopmental transcriptional program → impaired neuronal maturation, activity-dependent synaptic plasticity, and postnatal (dentate-gyrus) neurogenesis → intellectual disability with growth disruption and craniofacial dysmorphism. Conformers substitute the component and the mark it controls. Carries the chromatin-directed **HDAC-inhibitor** drug-target pattern (treatment uses `target_mechanisms` with `INHIBITS` on the chromatin-imbalance node), which rests on a single Kabuki mouse model and is **not** inherited as a treatment recommendation — recorded as an open `HUMAN_MODEL_MISMATCH`. DNA-methylation **episignatures** are deliberately NOT curated as a causal node (diagnostic biomarker, blood not brain; open `KNOWLEDGE_GAP`). Distinct from the aging-hallmark `epigenetic_alterations` module (age-associated drift, not a germline machinery lesion) and from transcription-factor disorders (FOXP1, SOX11), which do not conform on that basis alone. 20 conformers spanning all four classes — **writers** (Kabuki KMT2D/KDM6A, Rubinstein-Taybi CREBBP/EP300, Kleefstra EHMT1, Weaver EZH2, Wiedemann-Steiner KMT2A, SETD5 and Luscan-Lumish SETD2, Arboleda-Tham KAT6A), **erasers** (Beck-Fahrner TET3 — the DNA-methylation rather than histone arm, KDM6B, KDM1A), **remodelers** (Coffin-Siris and Nicolaides-Baraitser BAF, CHD8, CHD2, ATRX), **readers/scaffolds** (BRPF1, Börjeson-Forssman-Lehmann PHF6, Bainbridge-Ropers ASXL3), and cohesin (Cornelia de Lange NIPBL, attached only at the transcriptional node). Key conformance target: `epigenetic_machinery_neurodevelopmental_dysregulation#Dysregulated Neurodevelopmental Transcriptional Program`
