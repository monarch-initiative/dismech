...
| `docs/slides/*.html`, `docs/examples/*.html` | YES | Hand-authored or deliberately snapshotted illustrative HTML *published as documentation*, not build output. Distinct from the `docs/` row below, which means the mkdocs-rendered site. Keep these few and regenerate them via their own recipe (e.g. `just sepio-pilot` refreshes `docs/examples/sepio-evidence-pilot-page.html`) rather than by hand |
...
| `docs/` HTML output (the mkdocs-built site) | NO | Derived — regenerated by CI. Does not cover the authored/snapshotted `docs/slides/` and `docs/examples/` pages above |
