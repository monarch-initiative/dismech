...
def build_graph_data(disease: dict) -> dict:
    ...
    # Collect all perturbation target node IDs for JS
    all_perturbation_targets = set()
    for hg in result["hypothesis_groups"]:
        for t in hg.get("perturbation_targets", []):
            all_perturbation_targets.add(t)
    result["perturbation_target_nodes"] = sorted(all_perturbation_targets)
...
function layoutGraph(data) {{
  ...
  // Add free-flowing nodes (non-module) + perturbation targets
  const pertTargets = new Set(data.perturbation_target_nodes || []);
  data.nodes.filter(n => !moduleNodeIds.has(n.id) || pertTargets.has(n.id)).forEach(n => {{
    const w = Math.max(140, n.short_label.length * 8 + 30);
  ...
  // Add edges from perturbation targets to their module-internal neighbors
  data.edges.forEach(e => {{
    if (e.edge_context === "module_internal") {{
      // If source is a perturbation target, add edge from source to module panel
      if (pertTargets.has(e.source) && !pertTargets.has(e.target)) {{
        const tgtMod = nodeToModule[e.target];
        if (tgtMod) og.setEdge(e.source, "module:" + tgtMod);
      }}
      // If target is a perturbation target, add edge from module panel to target
      if (pertTargets.has(e.target) && !pertTargets.has(e.source)) {{
        const srcMod = nodeToModule[e.source];
        if (srcMod) og.setEdge("module:" + srcMod, e.target);
      }}
    }}
  }});
...
function drawModulePanels(container, og, moduleLayouts, nodeGroup, edgeGroup, nodeLabels, nodeMap) {{
  (DATA.modules || []).forEach(mod => {{
    ...
    if (isExpanded && moduleLayouts[mod.id]) {{
      ...
      // Draw internal nodes at translated positions (skip perturbation targets)
      const mg = ml.graph;
      const modNodeSet = new Set((DATA.module_nodes || {{}})[mod.id] || []);
      const pertTargets = new Set(DATA.perturbation_target_nodes || []);
      DATA.nodes.filter(n => modNodeSet.has(n.id) && !pertTargets.has(n.id)).forEach(n => {{
...
function drawEdges(og, moduleLayouts, edgeGroup, nodeLabels, moduleNodeIds, nodeToModule) {{
  ...
  // Helper: resolve a node position, using internal global position if in expanded module
  function resolvePos(nodeId) {{
    const inModule = moduleNodeIds.has(nodeId) && !pertTargets.has(nodeId);
    const modExpanded = inModule && _moduleState[nodeToModule[nodeId]] === "expanded";
    if (modExpanded) {{
      const nd = DATA.nodes.find(n => n.id === nodeId);
      if (nd && nd._gx !== undefined) return {{ x: nd._gx, y: nd._gy }};
    }}
    // Fall back to outer graph position
    const ogId = inModule ? "module:" + nodeToModule[nodeId] : nodeId;
    const ogPos = og.node(ogId);
    return ogPos ? {{ x: ogPos.x, y: ogPos.y }} : null;
  }}

  // Cross-boundary module edges
  DATA.edges.filter(e => e.edge_context !== "module_internal").forEach(e => {{
    const srcInModule = moduleNodeIds.has(e.source) && !pertTargets.has(e.source);
    const tgtInModule = moduleNodeIds.has(e.target) && !pertTargets.has(e.target);
    const srcModExpanded = srcInModule && _moduleState[nodeToModule[e.source]] === "expanded";
    const tgtModExpanded = tgtInModule && _moduleState[nodeToModule[e.target]] === "expanded";

    if (srcModExpanded || tgtModExpanded) {{
      const srcPos = resolvePos(e.source);
      const tgtPos = resolvePos(e.target);
      if (srcPos && tgtPos) {{
        drawSingleEdge(edgeGroup, [srcPos, tgtPos], e, nodeLabels, "solid");
      }}
    }} else {{
      const srcId = srcInModule ? "module:" + nodeToModule[e.source] : e.source;
      const tgtId = tgtInModule ? "module:" + nodeToModule[e.target] : e.target;
  ...
  (DATA.intermediate_edges || []).forEach(e => {{
    ...
    if (srcModExpanded || tgtModExpanded) {{
      ...
      const tgtPos = resolvePos(e.target);
      if (srcPos && tgtPos) {{
        drawSingleEdge(edgeGroup, [srcPos, tgtPos], e, nodeLabels, dashStyle);
      }}
    }} else {{
      const srcId = srcInModule ? "module:" + nodeToModule[e.source] : e.source;
      const tgtId = tgtInModule ? "module:" + nodeToModule[e.target] : e.target;
...
function render() {{
  ...
  // Panel backgrounds must be drawn FIRST so nodes/edges render on top
  const panelBgGroup = container.append("g").attr("class", "panel-backgrounds");
  const edgeGroup = container.append("g").attr("class", "edges");
  const nodeGroup = container.append("g").attr("class", "nodes");
  const nodeMap = {{}};

  drawModulePanels(panelBgGroup, og, moduleLayouts, nodeGroup, edgeGroup, nodeLabels, nodeMap);
