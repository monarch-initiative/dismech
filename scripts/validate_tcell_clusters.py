...
ENRICHMENT_FILE = Path("docs/assets/zhu_dann_2025/cluster_autoimmune_enrichment_results.csv")
...
def extract_genes_from_disorder(filepath: Path) -> dict:
    ...
    for entry in data.get("pathophysiology", []):
        ...
        for gene_entry in entry.get("genes", []):
            gname = gene_entry.get("name", "") if isinstance(gene_entry, dict) else str(gene_entry)
            if gname:
        ...
        for token in re.findall(r'\b([A-Z][A-Z0-9]{1,10})\b', text):
            ...
            if token in {"THE", "AND", "FOR", "WITH", "FROM", "THIS", "THAT",
                         "NOT", "BUT", "ARE", "HAS", "WAS", "MAY", "CAN",
                         "DNA", "RNA", "MHC", "HLA", "TNF", "NF", "IGG",
                         "IGE", "IGA", "IGM", "IFN", "TGF", "GWAS", "SNP",
                         "INCREASED", "DECREASED", "ABNORMAL", "VIA", "LOSS",
                         "TYPE", "CLASS"}:
                continue
...
def validate():
    ...
    for enr in enrichments:
        ...
        result = {
            ...
            "confirmation_rate": len(confirmed) / len(pipeline_genes) if pipeline_genes else 0,
        }
    ...
    for disease in sorted(results_by_disease.keys()):
        ...
        for r in sorted(entries, key=lambda x: -x["odds_ratio"]):
            ...
            print(f"\n  Cluster {r['cluster']} ({r['cluster_annotation']}) "
                  f"[{r['gene_set']}]")
            print(f"    OR={r['odds_ratio']:.1f}, FDR={r['fdr']:.2e}, "
            ...
            if r["novel_genes"]:
                ...
                suffix = f" ... +{len(r['novel_genes'])-15} more" if len(r["novel_genes"]) > 15 else ""
    ...
    print(f"Genes confirmed in dismech: {len(total_genes_confirmed)} "
          f"({100*len(total_genes_confirmed)/max(len(total_genes_tested),1):.1f}%)")
    print(f"Novel genes (not in dismech): {len(total_genes_novel)} "
    ...
    for disease in sorted(results_by_disease.keys()):
        ...
        print(f"  {disease:<33} {len(entries):>5} {len(all_conf):>10} "
