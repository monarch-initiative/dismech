...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Add PMID titles to disorder YAML files"
    )
    parser.add_argument(
        "files",
        nargs="*",
        help="Specific files to process (default: all kb/disorders/*.yaml)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for f in files:
        ...
        if stats["changed"]:
            ...
            if stats["ref_titles_added"] or stats["publications_restructured"]:
                print(
                    f"  {stats['file']}: +{stats['ref_titles_added']} titles, {stats['publications_restructured']} pubs restructured",
                    file=sys.stderr,
                )
        if stats["ref_titles_missing"] or stats["pub_titles_missing"]:
            print(
                f"  {stats['file']}: {stats['ref_titles_missing']} ref titles missing, {stats['pub_titles_missing']} pub titles missing",
                file=sys.stderr,
            )
    ...
    print(
        f"  Reference titles missing (no cache): {total_ref_missing}", file=sys.stderr
    )
    print(f"  Publications restructured: {total_pub_restructured}", file=sys.stderr)
    print(
        f"  Publication titles missing (no cache): {total_pub_missing}", file=sys.stderr
    )
