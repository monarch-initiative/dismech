    ...
    for entry in data.get("pathophysiology", []) or []:
        for gene_entry in entry.get("genes", []) or []:
            if isinstance(gene_entry, dict):
                gname = gene_entry.get("preferred_term", "") or gene_entry.get(
                    "name", ""
                )
            else:
...
def validate():
    ...
    print(
        f"Disorders referenced: {len(all_disorder_names)} "
    ...
    for trait_key in trait_order:
        ...
        trait_results[trait_key] = {
            ...
            "n_partial_gene": sum(
                1 for m in matches if m["category"] == "PARTIAL_GENE"
            ),
        }
    ...
    for trait_key in trait_order:
        ...
        print(
            f"  Disorders: {r['n_curated']}/{r['n_disorders']} curated, "
            f"{r['n_genes']} genes, {r['n_go_terms']} GO terms"
        )
        print(
            f"  Matches: {r['n_confirmed']} CONFIRMED, "
        ...
        for m in r["matches"]:
            go_labels = (
                [combined_go.get(g, g) for g in m["go_overlap"] if g in combined_go]
                if m["go_overlap"]
                else []
            )
            ...
            print(
                f"    {m['program']} ({m['annotation']}) [{m['category']}] "
                f"marker={m['marker']}"
            )
            if m["go_overlap"]:
    ...
    for pid in sorted(program_matches.keys(), key=lambda x: int(x[1:])):
        ...
        print(
            f"  {pid} ({pdata['curated_annotation']}) marker={pdata.get('marker_coexpression', 'none')}"
        )
    ...
    rbc_programs = {
        ...
        if pdata.get("marker_coexpression", "none")
        in ("RBC", "erythroid progenitor", "erythroid progenitor, RBC")
    }
    ...
    if unmatched_rbc:
        ...
        for pid in sorted(unmatched_rbc, key=lambda x: int(x[1:])):
            pdata = programs[pid]
            print(
                f"  {pid} ({pdata['curated_annotation']}) "
    ...
    if total_disorder_genes:
        ...
        print(
            f"Gene confirmation rate: {len(all_confirmed_genes)}/{len(total_disorder_genes)} "
            f"({rate:.1f}%)"
        )
